...
def run(args: list[str], cwd: Path | None = None, capture: bool = False) -> tuple[int, str, float]:
	...
	r = subprocess.run(args, cwd=cwd or ROOT, capture_output=capture, text=True, encoding="utf-8", errors="replace")
...
def main() -> int:
	...
	parser.add_argument("--debug", "-d", action="store_true", help="Build in debug mode (default: release)")
