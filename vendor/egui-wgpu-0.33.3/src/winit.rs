...
impl Painter {
    ...
    /// Returns `false` if the surface is temporarily unavailable (e.g. Vulkan
    /// ERROR_SURFACE_LOST_KHR during a resize on Wayland). Callers should
    /// skip work that depends on a valid surface and retry next frame.
    fn configure_surface(
        ...
        let Some(default_config) =
            ...
            log::warn!(
                "configure_surface: surface not supported at {width}x{height}, skipping (surface may have been lost)"
            );
            return false;
        };
    ...
    fn resize_and_generate_depth_texture_view_and_msaa_view(
        ...
        if !Self::configure_surface(surface_state, render_state, &self.configuration) {
            return;
        }
