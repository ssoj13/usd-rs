...
impl RenderState {
    ...
    pub async fn create(
        ...
        let target_format =
            crate::preferred_framebuffer_format(&surface_formats, &config.preferred_surface_formats)?;
        log::info!(
            "egui-wgpu surface formats: [{}] | preferred: [{}] | chosen: {:?}",
            surface_formats
                .iter()
...
pub struct WgpuConfiguration {
    ...
    /// Preferred window surface formats in descending order.
    ///
    /// The first format that is also supported by the surface will be used.
    /// Leave empty to keep egui-wgpu's upstream default preference order.
    pub preferred_surface_formats: Vec<wgpu::TextureFormat>,
...
impl std::fmt::Debug for WgpuConfiguration {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        f.debug_struct("WgpuConfiguration")
            ...
            .field("preferred_surface_formats", &preferred_surface_formats)
...
impl Default for WgpuConfiguration {
    fn default() -> Self {
        Self {
            ...
            preferred_surface_formats: Vec::new(),
