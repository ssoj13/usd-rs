...
| wrap | Метод | pxr | Статус |
|------|-------|--------|--------|
| L81–86 | `GetExtentAttr`, `CreateExtentAttr(defaultValue=…, writeSparsely=…)` | `GetExtentAttr`, `CreateExtentAttr(default_value=…, write_sparsely=…)`; ядро пишет default в `usd-geom::Boundable` | **OK** (2026-04) |
...
### 5.6 `UsdGeom.Gprim` — `Create*Attr` (displayColor / displayOpacity / doubleSided / orientation)

**Источник:** `wrapGprim.cpp`. **Ядро:** `crates/usd/usd-geom/src/gprim.rs`; **pxr:** `PyGprim`, делегаты на **`Gprim`** с `PointBased` / **Mesh** (`geom.rs`).

| Метод | Статус |
|-------|--------|
| `CreateDisplayColorAttr` / `CreateDisplayOpacityAttr` / `CreateOrientationAttr` — kwargs `defaultValue` / `writeSparsely` (PyO3: `default_value` / `write_sparsely`), default пишется в `Vt` на default time | **OK** (2026-04) |
| `CreateDoubleSidedAttr` — то же; **writeSparsely** + default `false` на **defined** prim пропускает авторинг (как прежний путь Mesh в pxr) | **OK** (2026-04) |

### 5.7 `UsdGeom.PointBased` — `CreateVelocitiesAttr` / `CreateNormalsAttr` / `CreateAccelerationsAttr`

**Источник:** `wrapPointBased.cpp`. **Ядро:** `crates/usd/usd-geom/src/point_based.rs` (default time sample при создании нового атрибута, без перезаписи при уже авторенном); **pxr:** `PyPointBased` в `geom.rs` — kwargs `default_value` / `write_sparsely` на **Velocities**, **Accelerations**, **Normals** (как у `CreatePointsAttr`).

| Метод | Статус |
|-------|--------|
| `CreateVelocitiesAttr` / `CreateNormalsAttr` / `CreateAccelerationsAttr` | **OK** (2026-04) |
...
Все пары из wrap (projection, aperture, clipping, fStop, focus, stereo, shutter, **exposure / exposureIso / exposureTime / exposureFStop / exposureResponsivity**) — **OK** по именам; **`Create*Attr`** — kwargs `default_value` / `write_sparsely` на **PyCamera**, default пишется в **`usd_geom::Camera`** через `create_camera_schema_attr` | **OK** (2026-04) для kwargs + default.
...
| `Create*Attr` kwargs | **OK** (2026-04) — ядро: default time при первом создании атрибута (`capsule.rs` и др.) |
...
| `Create*Attr` kwargs | **OK** (2026-04) — `PyCone` + ядро `cone.rs` (default time при создании) |
...
| `Create*Attr` kwargs | **OK** (2026-04) — `PyCube` + ядро `cube.rs` (`CreateSizeAttr` default time) |
...
**TetMesh (`CreateTetVertexIndicesAttr` / `CreateSurfaceFaceVertexIndicesAttr`):** kwargs уже в `PyTetMesh`; **ядро** `tet_mesh.rs` пишет `default_value` на default time при создании атрибута (2026-04).

**Gprim-примитивы (Cube, Sphere, Cone, Cylinder, Plane, Capsule — размеры, радиусы, ось и т.д.):** `Create*Attr(default_value=…)` в pxr; **ядро** соответствующих `cube.rs` / `sphere.rs` / `cone.rs` / `cylinder.rs` / `plane.rs` / `capsule.rs` пишет default на default time при **первом** создании атрибута (2026-04).

**Прочие схемы (`subset`, `curves`, `basis_curves`, `hermite_curves`, `nurbs_curves`, `mesh`, `nurbs_patch`, `point_instancer`):** общий хелпер `schema_create_default::apply_optional_default` в `usd-geom` — то же правило для `Create*Attr` (2026-04).
