# Реестр отклонений pxr_rs от OpenUSD Python (полный снимок для исправлений)

**Референс:** `C:\projects\projects.rust.cg\usd-refs\OpenUSD` (`pxr` / `wrap*.cpp`, `tf/*.py`, тесты `pxr/...`).  
**Порт:** `crates/usd-pyo3`, пакет `pxr_rs`.  
...
**Реализация pxr_rs:** `crates/usd-pyo3/src/geom.rs` (`PyBBoxCache`), низлежащий тип `usd_geom::BBoxCache`.
...
| wrap (строки) | C++ `boost::python` | pxr_rs | Статус / отклонение |
|---------------|---------------------|--------|---------------------|
...
| wrap (строки) | Метод C++ / Python pxr | pxr_rs | Статус |
|---------------|--------------------------|--------|--------|
...
| API | Примечание |
|-----|------------|
| `GetIgnoreVisibility()` | В **заголовке** C++ есть (`bboxCache.h`); в **wrapBBoxCache.cpp** к классу **не** добавлен `.def("GetIgnoreVisibility", ...)`. Официальный Python pxr может не экспортировать это — перед добавлением в pxr_rs сверить с установленным у пользователя `pxr`. |
...
**pxr_rs:** `crates/usd-pyo3/src/geom.rs` (`PyXformCache`), низлежащий `usd_geom::XformCache`.
...
| wrap (строки) | C++ / pxr Python | pxr_rs | Статус |
|---------------|------------------|--------|--------|
...
**pxr_rs:** `crates/usd-pyo3/src/geom.rs` (`PyBasisCurves`); логика в `crates/usd/usd-geom/src/basis_curves.rs` (`BasisCurves`).

В C++ `class_<UsdGeomBasisCurves, bases<UsdGeomCurves>>` — в Python **BasisCurves наследует Curves** (методы `GetCurveVertexCountsAttr`, `ComputeExtent`, цепочка к `PointBased` / `Gprim` / `Xformable` и т.д.). В PyO3 **`PyBasisCurves` не подкласс `PyCurves`** — отдельная проблема наследования для всех схем (см. также §11 XF3).

### 4.1 Конструкторы

| wrap (строки) | C++ / pxr | pxr_rs | Статус |
|---------------|-----------|--------|--------|
...
| wrap | C++ / pxr | pxr_rs | Статус |
|------|-------------|--------|--------|
| L79–80 | `Get(stage, path)` | `Get` | **OK** (`path`: `str` \| `Sdf.Path`) |
| L82–83 | `Define(stage, path)` | `Define` | **OK** |
| L85–89 | `GetSchemaAttributeNames(includeInherited=True)` → `list` | — | **MISSING (P1)** — в Rust есть `BasisCurves::get_schema_attribute_names` |
| L91–93 | `_GetStaticTfType()` | — | **MISSING (P2)** — как и у других схем без Tf-обёртки |

### 4.3 Атрибуты Type / Basis / Wrap

| wrap | Метод | pxr_rs | Статус |
|------|-------|--------|--------|
| L98–103 | `GetTypeAttr`, `CreateTypeAttr(defaultValue=…, writeSparsely=…)` | `GetTypeAttr`, `CreateTypeAttr` **без** kwargs — внутри `None, false` | **PARTIAL (P2)** — нет передачи `defaultValue` / `writeSparsely` как в C++ |
| L105–110 | `GetBasisAttr`, `CreateBasisAttr(...)` | то же | **PARTIAL (P2)** |
| L112–117 | `GetWrapAttr`, `CreateWrapAttr(...)` | то же | **PARTIAL (P2)** |

### 4.4 Вспомогательные методы (кастомный блок WRAP_CUSTOM, L157–167)

Реализация в **`usd-geom`** (`compute_interpolation_for_size`, `compute_uniform_data_size`, …) есть; в **`PyBasisCurves` не экспортированы**.

| wrap | C++ / pxr | pxr_rs | Статус |
|------|-------------|--------|--------|
| L162 | `ComputeInterpolationForSize(n, time: UsdTimeCode)` → `Tf.Token` | — | **MISSING (P1)** |
| L163 | `ComputeUniformDataSize(time)` | — | **MISSING (P1)** |
| L164 | `ComputeVaryingDataSize(time)` | — | **MISSING (P1)** |
| L165 | `ComputeVertexDataSize(time)` | — | **MISSING (P1)** |
| L166 | `ComputeSegmentCounts(time)` → `list[int]` | — | **MISSING (P1)** |
...
| BC-SCH | **`GetSchemaAttributeNames`** |
| BC-COMPUTE | **`ComputeInterpolationForSize`**, **`ComputeUniformDataSize`**, **`ComputeVaryingDataSize`**, **`ComputeVertexDataSize`**, **`ComputeSegmentCounts`** + время `Usd.TimeCode` |
...
**pxr_rs:** `crates/usd-pyo3/src/geom.rs` (`PyBoundable`); логика — `crates/usd/usd-geom/src/boundable.rs`.
...
**pxr_rs:** `crates/usd-pyo3/src/geom.rs` (`PyCamera`); ядро — `crates/usd/usd-geom/src/camera.rs`. **`Gf.Camera`** — `crates/usd-pyo3/src/gf/geo.rs` (`pxr_rs.Gf.Camera`).
...
**pxr_rs:** `PyCapsule` / `PyCapsule1` в `geom.rs`; ядро — `usd-geom` `capsule.rs`.
...
**pxr_rs:** `crates/usd-pyo3/src/geom.rs` (`PyCone`); ядро — `crates/usd/usd-geom/src/cone.rs`.

| Элемент | pxr_rs | Статус |
|---------|--------|--------|
| `Get`, `Define` | есть | **OK** |
...
**pxr_rs:** `crates/usd-pyo3/src/geom.rs` (`PyConstraintTarget`); ядро — `crates/usd/usd-geom/src/constraint_target.rs`.
...
**pxr_rs:** `crates/usd-pyo3/src/geom.rs` (`PyCube`); ядро — `crates/usd/usd-geom/src/cube.rs`.
...
| XF3 | Делегирование **Imageable/Xformable** не на всех схемах (Mesh, Camera, …) — тесты «только у Xform» vs полный паритет. | `PYTHON_API_PARITY.md` | P1 |
...
Во многих местах pxr_rs возвращает **плоские списки** (`Vec<f64>`) вместо **Gf-типов** (`Gf.Matrix4d`, `Gf.BBox3d`, `Gf.Vec3d`). Это:
...
- [ ] `pxr/usd/usdGeom/wrap*.cpp` — построчно **остальные** файлы (алфавит: далее детальная сверка `wrapCurves.cpp`, `wrapMesh.cpp`, … — все методы класса, не только статика).  
...
**Реализация pxr_rs:** `crates/usd-pyo3/src/geom.rs` — делегирование в `usd_geom::*::get_schema_attribute_names(include_inherited)` и `#[pyo3(name = "GetSchemaAttributeNames", signature = (include_inherited = true))]` где применимо.
