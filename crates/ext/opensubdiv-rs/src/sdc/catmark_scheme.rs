...
impl SchemeKernel for CatmarkKernel {
    ...
    fn assign_smooth_mask_for_edge<E: EdgeNeighborhood, M: MaskInterface>(
        ...
        if !use_tri_opt {
            ...
            let f0w: Weight = if face0_is_tri {
                CATMARK_SMOOTH_TRI_EDGE_WEIGHT
            } else {
                0.25
            };
            let f1w: Weight = if face1_is_tri {
                CATMARK_SMOOTH_TRI_EDGE_WEIGHT
            } else {
                0.25
            };
    ...
    fn assign_crease_limit_tangent_masks<V: VertexNeighborhood, M: MaskInterface>(
        ...
        if interior == 1 {
            ...
            for i in 1..(k as usize) {
                ...
                tan2.set_edge_weight(crease_ends[0] + i, (4.0 * sin_i * common_denom) as Weight);
...
mod tests {
    ...
    fn edge_smooth_tri_smooth_option() {
        ...
        let vw = 0.5 * (1.0 - 2.0 * fw);
        assert!(
            (mask.vertex_weight(0) - vw).abs() < 1e-5,
            "vw = {}",
            mask.vertex_weight(0)
        );
        assert!(
            (mask.face_weight(0) - fw).abs() < 1e-5,
            "fw = {}",
            mask.face_weight(0)
        );
    }
    ...
    fn vertex_smooth_valence3() {
        ...
        let sum = mask.vertex_weight(0)
            + (0..3)
                .map(|i| mask.edge_weight(i) + mask.face_weight(i))
                .sum::<f32>();
    ...
    fn limit_crease_weights() {
        ...
        CatmarkKernel::assign_crease_limit_mask(&Options::default(), &v, &mut mask, [0, 2]);
