...
mod tests {
    ...
    impl VertexNeighborhood for SmoothVertex {
        ...
        fn child_sharpness(&self, _: &Crease) -> f32 {
            0.0
        }
    ...
    fn edge_smooth_opposite_vertex_weights() {
        ...
        assert!(
            (mask.vertex_weight(0) - 0.375).abs() < 1e-6,
            "vw = {}",
            mask.vertex_weight(0)
        );
        assert!((mask.vertex_weight(1) - 0.375).abs() < 1e-6);
        assert!(
            (mask.face_weight(0) - 0.125).abs() < 1e-6,
            "fw = {}",
            mask.face_weight(0)
        );
    ...
    fn edge_smooth_flag_always_reset_to_false() {
        ...
        assert!(
            (mask.vertex_weight(0) - 0.375).abs() < 1e-6,
            "vw = {} (expected 0.375, flag must be overridden to false)",
            mask.vertex_weight(0)
        );
        assert!(
            (mask.face_weight(0) - 0.125).abs() < 1e-6,
            "fw = {} (expected 0.125, flag must be overridden to false)",
            mask.face_weight(0)
        );
    ...
    fn vertex_smooth_regular_valence6() {
        ...
        for i in 0..6 {
            assert!(
                (mask.edge_weight(i) - 0.0625).abs() < 1e-6,
                "edge[{}] = {}",
                i,
                mask.edge_weight(i)
            );
        }
        let sum = mask.vertex_weight(0) + (0..6).map(|i| mask.edge_weight(i)).sum::<f32>();
    ...
    fn vertex_smooth_irregular_valence5() {
        ...
        let sum = mask.vertex_weight(0) + (0..5).map(|i| mask.edge_weight(i)).sum::<f32>();
        assert!((sum - 1.0).abs() < 1e-5, "sum = {}", sum);
    ...
    fn limit_crease_sums_to_one() {
        ...
        LoopKernel::assign_crease_limit_mask(&Options::default(), &v, &mut mask, [0, 2]);
        ...
        let sum = mask.vertex_weight(0) + (0..4).map(|i| mask.edge_weight(i)).sum::<f32>();
    ...
    fn tangent_smooth_regular_valence6() {
        ...
        for (i, &e) in expected_tan1.iter().enumerate() {
            assert!(
                (tan1.edge_weight(i) - e).abs() < 1e-5,
                "tan1 edge[{}]: got {}, expected {}",
                i,
                tan1.edge_weight(i),
                e
            );
