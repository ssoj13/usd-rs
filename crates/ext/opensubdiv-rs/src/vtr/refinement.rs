...
use crate::sdc::{
    Options,
    crease::{Crease, Rule, SHARPNESS_SMOOTH, SHARPNESS_INFINITE, is_sharp},
    types::Split,
};
...
impl Relations {
    pub fn all_true() -> Self {
        Relations {
            face_vertices: true, face_edges: true,
            edge_vertices: true, edge_faces: true,
            vertex_faces: true,  vertex_edges: true,
        }
    }
    pub fn all_false() -> Self {
        Relations {
            face_vertices: false, face_edges: false,
            edge_vertices: false, edge_faces: false,
            vertex_faces: false,  vertex_edges: false,
        }
    }
}
...
#[inline]
pub fn is_sparse_index_marked(index: Index) -> bool { index != 0 }

pub const SPARSE_MASK_NEIGHBORING: Index = 1 << 0;
pub const SPARSE_MASK_SELECTED:    Index = 1 << 1;

#[inline] pub fn mark_sparse_neighbor(idx: &mut Index) { *idx = SPARSE_MASK_NEIGHBORING; }
#[inline] pub fn mark_sparse_selected(idx: &mut Index) { *idx = SPARSE_MASK_SELECTED; }
/// Alias so quad_refinement can call it by a consistent name.
#[inline] pub fn mark_sparse_selected_pub(idx: &mut Index) { *idx = SPARSE_MASK_SELECTED; }

#[inline]
fn sequence_sparse_index_vector(v: &mut Vec<Index>, base: i32) -> i32 {
    let mut count = 0i32;
    for x in v.iter_mut() {
        *x = if is_sparse_index_marked(*x) { let r = base + count; count += 1; r } else { INDEX_INVALID };
    }
    count
}
...
fn sequence_full_index_vector(v: &mut Vec<Index>, base: i32) -> i32 {
    let n = v.len() as i32;
    for (i, x) in v.iter_mut().enumerate() { *x = base + i as i32; }
    n
}
...
impl Refinement {
    ...
    pub unsafe fn new(parent: *const Level, child: *mut Level, options: Options,
                      split_type: Split) -> Self
    {
        // Set child depth immediately, as C++ does in the ctor
        unsafe { (*child).depth = 1 + (*parent).depth; }
    ...
    #[inline] pub fn parent(&self) -> &Level { unsafe { &*self.parent } }
    #[inline] pub fn child(&self)  -> &Level { unsafe { &*self.child  } }
    #[inline] pub fn child_mut(&mut self) -> &mut Level { unsafe { &mut *self.child } }

    // ---- basic accessors ----
    #[inline] pub fn get_split_type(&self)       -> Split   { self.split_type }
    #[inline] pub fn get_regular_face_size(&self) -> i32    { self.reg_face_size }
    #[inline] pub fn get_options(&self)           -> Options { self.options }
    #[inline] pub fn has_face_vertices_first(&self) -> bool { self.face_verts_first }

    // ---- child component counts ----
    #[inline] pub fn get_num_child_faces_from_faces(&self)       -> i32 { self.child_face_from_face_count }
    #[inline] pub fn get_num_child_edges_from_faces(&self)       -> i32 { self.child_edge_from_face_count }
    #[inline] pub fn get_num_child_edges_from_edges(&self)       -> i32 { self.child_edge_from_edge_count }
    #[inline] pub fn get_num_child_vertices_from_faces(&self)    -> i32 { self.child_vert_from_face_count }
    #[inline] pub fn get_num_child_vertices_from_edges(&self)    -> i32 { self.child_vert_from_edge_count }
    #[inline] pub fn get_num_child_vertices_from_vertices(&self) -> i32 { self.child_vert_from_vert_count }

    #[inline] pub fn get_first_child_face_from_faces(&self)      -> Index { self.first_child_face_from_face }
    #[inline] pub fn get_first_child_edge_from_faces(&self)      -> Index { self.first_child_edge_from_face }
    #[inline] pub fn get_first_child_edge_from_edges(&self)      -> Index { self.first_child_edge_from_edge }
    #[inline] pub fn get_first_child_vertex_from_faces(&self)    -> Index { self.first_child_vert_from_face }
    #[inline] pub fn get_first_child_vertex_from_edges(&self)    -> Index { self.first_child_vert_from_edge }
    #[inline] pub fn get_first_child_vertex_from_vertices(&self) -> Index { self.first_child_vert_from_vert }

    // ---- parent → child vertex accessors ----
    #[inline] pub fn get_face_child_vertex(&self, f: Index)   -> Index { self.face_child_vert_index[f as usize] }
    #[inline] pub fn get_edge_child_vertex(&self, e: Index)   -> Index { self.edge_child_vert_index[e as usize] }
    #[inline] pub fn get_vertex_child_vertex(&self, v: Index) -> Index { self.vert_child_vert_index[v as usize] }
    ...
    pub fn get_edge_child_edges(&self, e: Index) -> &[Index; 2] {
        ...
        self.edge_child_edge_indices[off..off + 2].try_into().unwrap()
    }
    ...
    #[inline] pub fn get_child_face_parent_face(&self, f: Index)    -> Index { self.child_face_parent_index[f as usize] }
    #[inline] pub fn get_child_face_in_parent_face(&self, f: Index) -> i32   { self.child_face_tag[f as usize].index_in_parent as i32 }
    #[inline] pub fn get_child_edge_parent_index(&self, e: Index)   -> Index { self.child_edge_parent_index[e as usize] }
    #[inline] pub fn get_child_vertex_parent_index(&self, v: Index) -> Index { self.child_vertex_parent_index[v as usize] }
    #[inline] pub fn is_child_vertex_complete(&self, v: Index)      -> bool  { !self.child_vertex_tag[v as usize].incomplete }

    // ---- child tag accessors ----
    #[inline] pub fn get_child_face_tag(&self, f: Index)   -> &ChildTag { &self.child_face_tag[f as usize] }
    #[inline] pub fn get_child_edge_tag(&self, e: Index)   -> &ChildTag { &self.child_edge_tag[e as usize] }
    #[inline] pub fn get_child_vertex_tag(&self, v: Index) -> &ChildTag { &self.child_vertex_tag[v as usize] }

    #[inline] pub fn get_child_face_tag_mut(&mut self, f: Index)   -> &mut ChildTag { &mut self.child_face_tag[f as usize] }
    #[inline] pub fn get_child_edge_tag_mut(&mut self, e: Index)   -> &mut ChildTag { &mut self.child_edge_tag[e as usize] }
    #[inline] pub fn get_child_vertex_tag_mut(&mut self, v: Index) -> &mut ChildTag { &mut self.child_vertex_tag[v as usize] }

    // ---- sparse tag accessors ----
    #[inline] pub fn get_parent_face_sparse_tag(&self, f: Index)     -> &SparseTag { &self.parent_face_tag[f as usize] }
    #[inline] pub fn get_parent_edge_sparse_tag(&self, e: Index)     -> &SparseTag { &self.parent_edge_tag[e as usize] }
    #[inline] pub fn get_parent_vertex_sparse_tag(&self, v: Index)   -> &SparseTag { &self.parent_vertex_tag[v as usize] }

    #[inline] pub fn get_parent_face_sparse_tag_mut(&mut self, f: Index)   -> &mut SparseTag { &mut self.parent_face_tag[f as usize] }
    #[inline] pub fn get_parent_edge_sparse_tag_mut(&mut self, e: Index)   -> &mut SparseTag { &mut self.parent_edge_tag[e as usize] }
    #[inline] pub fn get_parent_vertex_sparse_tag_mut(&mut self, v: Index) -> &mut SparseTag { &mut self.parent_vertex_tag[v as usize] }

    // ---- FVar channel accessors ----
    #[inline] pub fn get_num_fvar_channels(&self) -> i32 { self.fvar_channels.len() as i32 }
    #[inline] pub fn get_fvar_refinement(&self, c: i32) -> &FVarRefinement { &self.fvar_channels[c as usize] }
    ...
    pub fn refine(&mut self, opts: RefinementOptions) {
        ...
        if has_fvar { relations.vertex_faces = true; }

        if relations.face_vertices { (self.populate_fv_fn.unwrap())(self); }
        if relations.face_edges    { (self.populate_fe_fn.unwrap())(self); }
        if relations.edge_vertices { (self.populate_ev_fn.unwrap())(self); }
        if relations.edge_faces    { (self.populate_ef_fn.unwrap())(self); }
        if relations.vertex_faces  { (self.populate_vf_fn.unwrap())(self); }
        if relations.vertex_edges  { (self.populate_ve_fn.unwrap())(self); }
        ...
        if has_fvar { self.subdivide_fvar_channels(); }
    }
    // =========================================================================
    ...
    fn populate_parent_child_indices(&mut self) {
        if self.uniform {
            ...
            self.child_face_from_face_count =
                sequence_full_index_vector(&mut self.face_child_face_indices, self.first_child_face_from_face);

            self.first_child_edge_from_face = 0;
            self.child_edge_from_face_count =
                sequence_full_index_vector(&mut self.face_child_edge_indices, self.first_child_edge_from_face);

            self.first_child_edge_from_edge = self.child_edge_from_face_count;
            self.child_edge_from_edge_count =
                sequence_full_index_vector(&mut self.edge_child_edge_indices, self.first_child_edge_from_edge);

            if self.face_verts_first {
                self.first_child_vert_from_face = 0;
                self.child_vert_from_face_count =
                    sequence_full_index_vector(&mut self.face_child_vert_index, self.first_child_vert_from_face);

                self.first_child_vert_from_edge = self.first_child_vert_from_face + self.child_vert_from_face_count;
                self.child_vert_from_edge_count =
                    sequence_full_index_vector(&mut self.edge_child_vert_index, self.first_child_vert_from_edge);

                self.first_child_vert_from_vert = self.first_child_vert_from_edge + self.child_vert_from_edge_count;
                self.child_vert_from_vert_count =
                    sequence_full_index_vector(&mut self.vert_child_vert_index, self.first_child_vert_from_vert);
            } else {
                self.first_child_vert_from_vert = 0;
                self.child_vert_from_vert_count =
                    sequence_full_index_vector(&mut self.vert_child_vert_index, self.first_child_vert_from_vert);

                self.first_child_vert_from_face = self.first_child_vert_from_vert + self.child_vert_from_vert_count;
                self.child_vert_from_face_count =
                    sequence_full_index_vector(&mut self.face_child_vert_index, self.first_child_vert_from_face);
            ...
            self.first_child_face_from_face = 0;
            self.child_face_from_face_count =
                sequence_sparse_index_vector(&mut self.face_child_face_indices, self.first_child_face_from_face);

            self.first_child_edge_from_face = 0;
            self.child_edge_from_face_count =
                sequence_sparse_index_vector(&mut self.face_child_edge_indices, self.first_child_edge_from_face);

            self.first_child_edge_from_edge = self.child_edge_from_face_count;
            self.child_edge_from_edge_count =
                sequence_sparse_index_vector(&mut self.edge_child_edge_indices, self.first_child_edge_from_edge);

            if self.face_verts_first {
                self.first_child_vert_from_face = 0;
                self.child_vert_from_face_count =
                    sequence_sparse_index_vector(&mut self.face_child_vert_index, self.first_child_vert_from_face);

                self.first_child_vert_from_edge = self.first_child_vert_from_face + self.child_vert_from_face_count;
                self.child_vert_from_edge_count =
                    sequence_sparse_index_vector(&mut self.edge_child_vert_index, self.first_child_vert_from_edge);

                self.first_child_vert_from_vert = self.first_child_vert_from_edge + self.child_vert_from_edge_count;
                self.child_vert_from_vert_count =
                    sequence_sparse_index_vector(&mut self.vert_child_vert_index, self.first_child_vert_from_vert);
            } else {
                self.first_child_vert_from_vert = 0;
                self.child_vert_from_vert_count =
                    sequence_sparse_index_vector(&mut self.vert_child_vert_index, self.first_child_vert_from_vert);
    ...
    pub fn initialize_child_component_counts(&mut self) {
        ...
        child.vert_count = self.child_vert_from_face_count + self.child_vert_from_edge_count + self.child_vert_from_vert_count;
    }
            ...
            for p_face in 0..self.parent().get_num_faces() {
                ...
                for (i, &cf) in c_faces.iter().enumerate() {
                    if n == 4 {
                        ...
                        self.child_face_tag[c_face as usize] = tags[0][if too_large { 0 } else { i }];
                    }
                ...
                if !incomplete && n == 4 {
                    ...
                    for (i, &cf) in c_faces.iter().enumerate() {
                        if index_is_valid(cf) {
                            self.child_face_tag[cf as usize] = tags[if incomplete { 1 } else { 0 }][if too_large { 0 } else { i }];
                            self.child_face_parent_index[cf as usize] = p_face;
                        }
                    }
                }
                ...
                if !incomplete && n == 4 {
                    ...
                    for (i, &ce) in c_edges.iter().enumerate() {
                        if index_is_valid(ce) {
                            self.child_edge_tag[ce as usize] = tags[if incomplete { 1 } else { 0 }][if too_large { 0 } else { i }];
                            self.child_edge_parent_index[ce as usize] = p_face;
                        }
                    }
                }
    ...
    fn populate_vertex_parent_from_parent_faces(&mut self, tags: &[[ChildTag; 4]; 2]) {
        if self.child_vert_from_face_count == 0 { return; }
        ...
        if !self.uniform {
            ...
            for c_vert in 0..ncv {
                if self.child_vertex_tag[c_vert as usize].incomplete {
                    unsafe { let child = &mut *self.child; child.vert_tags[c_vert as usize].set_incomplete(true); }
                }
            }
        }
    }
    ...
    pub fn subdivide_topology(&mut self, apply_to: Relations) {
        if apply_to.face_vertices { if let Some(f) = self.populate_fv_fn { f(self); } }
        if apply_to.face_edges    { if let Some(f) = self.populate_fe_fn { f(self); } }
        if apply_to.edge_vertices { if let Some(f) = self.populate_ev_fn { f(self); } }
        if apply_to.edge_faces    { if let Some(f) = self.populate_ef_fn { f(self); } }
        if apply_to.vertex_faces  { if let Some(f) = self.populate_vf_fn { f(self); } }
        if apply_to.vertex_edges  { if let Some(f) = self.populate_ve_fn { f(self); } }
    ...
    pub(crate) fn allocate_parent_child_indices(&mut self) {
        if let Some(f) = self.allocate_fn { f(self); }
    }
    ...
    pub(crate) fn mark_sparse_face_children(&mut self) {
        if let Some(f) = self.sparse_face_fn { f(self); }
    }
    ...
    fn subdivide_edge_sharpness(&mut self) {
        ...
        unsafe { (&mut *self.child).edge_sharpness.resize(nce, SHARPNESS_SMOOTH) };
            ...
            unsafe {
                ...
                if c_et.inf_sharp() {
                    ...
                    let new_sharp = if crease.is_uniform() {
                        ...
                        let sharp_buf: Vec<f32> = p_vert_edges.as_slice()
                            .iter().map(|&e| (&*self.parent).edge_sharpness[e as usize]).collect();
    ...
    fn subdivide_vertex_sharpness(&mut self) {
        ...
        unsafe { (&mut *self.child).vert_sharpness.resize(ncv, SHARPNESS_SMOOTH) };
    ...
    fn reclassify_semisharp_vertices(&mut self) {
        ...
        for c_vert in from_edge_start..from_edge_end {
            unsafe {
                ...
                if !c_vt.semi_sharp_edges() { continue; }

                let p_edge  = self.child_vertex_parent_index[c_vert as usize];
                let c_edges = *self.get_edge_child_edges(p_edge);

                if self.child_vertex_tag[c_vert as usize].incomplete {
                    let semi = (index_is_valid(c_edges[0]) && (&*self.child).edge_tags[c_edges[0] as usize].semi_sharp())
                             | (index_is_valid(c_edges[1]) && (&*self.child).edge_tags[c_edges[1] as usize].semi_sharp());
                    (&mut (*self.child).vert_tags)[c_vert as usize].set_semi_sharp_edges(semi);
                    (&mut (*self.child).vert_tags)[c_vert as usize].set_rule(
                        if semi { Rule::Crease as u16 } else { Rule::Smooth as u16 });
                } else {
                    let sharp_count =
                        (&*self.child).edge_tags[c_edges[0] as usize].semi_sharp() as i32
                      + (&*self.child).edge_tags[c_edges[1] as usize].semi_sharp() as i32;
                    (&mut (*self.child).vert_tags)[c_vert as usize].set_semi_sharp_edges(sharp_count > 0);
        ...
        for c_vert in from_vert_start..from_vert_end {
            unsafe {
                ...
                if !p_vtag_semi && !p_vtag_semi_edges { continue; }
                ...
                if p_vtag_semi_edges || sharp_decayed {
                    ...
                    if (&*self.child).get_num_vertex_edges_total() > 0 {
                        ...
                        let c_edges_vec: Vec<Index> = (&*self.child).get_vertex_edges(c_vert)
                            .as_slice().to_vec();
                        ...
                        let p_edges_vec: Vec<Index> = (&*self.parent).get_vertex_edges(p_vert)
                            .as_slice().to_vec();
                        let p_local_vec: Vec<super::types::LocalIndex> = (&*self.parent).get_vertex_edge_local_indices(p_vert)
                            .as_slice().to_vec();
                    ...
                    (&mut (*self.child).vert_tags)[c_vert as usize].set_semi_sharp_edges(semi_count > 0);
                    if !c_vt.semi_sharp() && !c_vt.inf_sharp() {
                        let rule = crease.determine_vertex_vertex_rule_from_count(0.0, inf_count + semi_count);
                        (&mut (*self.child).vert_tags)[c_vert as usize].set_rule(rule as u16);
                    }
                }
            }
    ...
    fn mark_sparse_edge_children(&mut self) {
        ...
        for p_edge in 0..self.parent().get_num_edges() {
            ...
            let transitional = if e_faces.size() == 2 {
                ...
                let trans = e_faces.as_slice()[1..].iter()
    ...
    fn face_child_face_co(&self, f: Index) -> (usize, usize) {
        if self.face_child_face_counts_offsets_shared {
            ...
            let count = unsafe {
                (*self.parent).get_num_face_vertices(f) as usize
            };
            let offset = unsafe {
                (*self.parent).get_offset_of_face_vertices(f) as usize
            };
            (count, offset)
        }
    }
    ...
    pub fn print_parent_to_child_mapping(&self) {
        ...
        for p_face in 0..p.get_num_faces() {
            println!("  Face {}:", p_face);
            println!("    Child vert:  {}", self.face_child_vert_index[p_face as usize]);
        ...
        for p_edge in 0..p.get_num_edges() {
            ...
            println!("  Edge {}: vert={}, edges=[{}, {}]", p_edge,
                self.edge_child_vert_index[p_edge as usize], ce[0], ce[1]);
        }
        for p_vert in 0..p.get_num_vertices() {
            println!("  Vert {}: child_vert={}", p_vert, self.vert_child_vert_index[p_vert as usize]);
        }
    }
    ...
    pub fn refine_with_callbacks(
        ...
        if relations.face_vertices { populate_fv(self); }
        if relations.face_edges    { populate_fe(self); }
        if relations.edge_vertices { populate_ev(self); }
        if relations.edge_faces    { populate_ef(self); }
        if relations.vertex_faces  { populate_vf(self); }
        if relations.vertex_edges  { populate_ve(self); }
...
mod tests {
    ...
    fn relations_all_true() {
        ...
        assert!(r.face_vertices && r.face_edges && r.edge_vertices &&
                r.edge_faces && r.vertex_faces && r.vertex_edges);
    }
}
