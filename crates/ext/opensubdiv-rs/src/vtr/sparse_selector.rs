...
impl<'r> SparseSelector<'r> {
    ...
    pub fn get_refinement(&self) -> &Refinement {
        self.refinement
    }
    pub fn get_refinement_mut(&mut self) -> &mut Refinement {
        self.refinement
    }
    ...
    pub fn select_face(&mut self, f: Index) {
        ...
        if !self.was_face_selected(f) {
            ...
            let face_edges: Vec<Index> = self
                .refinement
                .parent()
                .get_face_edges(f)
                .as_slice()
                .to_vec();
            let face_verts: Vec<Index> = self
                .refinement
                .parent()
                .get_face_vertices(f)
                .as_slice()
