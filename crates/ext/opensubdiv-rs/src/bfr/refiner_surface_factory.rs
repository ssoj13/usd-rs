...
pub trait TopologyRefinerMesh {
    ...
    fn corner_incident_fvar_indices(&self, face: Index, corner: i32, channel: i32, out: &mut [Index]) -> i32;
    ...
    fn neighborhood_fvar_indices_if_regular(&self, face: Index, channel: i32, out: Option<&mut [Index]>) -> bool {
...
impl<'mesh, M: TopologyRefinerMesh> RefinerSurfaceFactoryBase<'mesh, M> {
    pub fn new(mesh: &'mesh M, options: SurfaceFactoryOptions) -> Self {
        let factory = SurfaceFactory::new(
            mesh.scheme_type(),
            mesh.scheme_options(),
            options,
        );
        ...
        RefinerSurfaceFactoryBase { factory, mesh, num_faces, num_fvar_channels }
    }

    pub fn get_mesh(&self)              -> &M  { self.mesh }
    pub fn get_num_faces(&self)         -> i32 { self.num_faces }
    pub fn get_num_fvar_channels(&self) -> i32 { self.num_fvar_channels }
    ...
    for RefinerSurfaceFactoryBase<'mesh, M>
{
    ...
    fn populate_face_vertex_descriptor(
        ...
        self.mesh.populate_corner_descriptor(face_index, face_vertex, descriptor)
    }
    ...
    fn get_face_vertex_incident_face_vertex_indices(
        ...
        self.mesh.corner_incident_vtx_indices(face_index, face_vertex, out)
    }
    ...
    fn get_face_vertex_incident_face_fvar_value_indices(
        ...
        let ch = self.fvar_id_to_channel(fvar_id);
        if ch < 0 { return 0; }
        self.mesh.corner_incident_fvar_indices(face_index, face_vertex, ch, out)
    }
    ...
    fn get_face_neighborhood_vertex_indices_if_regular(
        ...
        self.mesh.neighborhood_vtx_indices_if_regular(face_index, vertex_indices)
    }
    ...
    fn get_face_neighborhood_fvar_value_indices_if_regular(
        ...
        self.mesh.neighborhood_fvar_indices_if_regular(face_index, ch, fvar_indices)
    }
}
...
impl<'mesh, M: TopologyRefinerMesh> RefinerSurfaceFactory<'mesh, M> {
    pub fn new(mesh: &'mesh M, options: SurfaceFactoryOptions) -> Self {
        let cache = std::sync::Arc::new(
            std::sync::RwLock::new(SurfaceFactoryCache::new()));
        let mut base = RefinerSurfaceFactoryBase::new(mesh, options);
        base.factory.set_internal_cache(cache.clone() as std::sync::Arc<dyn super::surface_factory_cache::SurfaceFactoryCacheTrait>);
        RefinerSurfaceFactory { base, _cache: cache }
    }
    ...
    pub fn init_vertex_surface<R: super::surface::SurfaceReal>(
        ...
        self.base.factory.init_vertex_surface(&self.base, face_index, surface)
    }
    ...
    pub fn init_varying_surface<R: super::surface::SurfaceReal>(
        ...
        self.base.factory.init_varying_surface(&self.base, face_index, surface)
    }
    ...
    pub fn init_fvar_surface<R: super::surface::SurfaceReal>(
        ...
        self.base.factory.init_fvar_surface(&self.base, face_index, fvar_id, surface)
    }

    /// Returns true if the face has a limit surface.
    pub fn face_has_limit_surface(&self, face_index: Index) -> bool {
        self.base.factory.face_has_limit_surface(&self.base, face_index)
    }

    /// Returns the parameterization for the given face.
    pub fn get_face_parameterization(&self, face_index: Index) -> super::parameterization::Parameterization {
        self.base.factory.get_face_parameterization(&self.base, face_index)
    }

    /// Return the mesh this factory was constructed for.
    pub fn get_mesh(&self) -> &M { self.base.get_mesh() }

    /// Number of faces at the base level.
    pub fn get_num_faces(&self) -> i32 { self.base.get_num_faces() }

    /// Number of face-varying channels.
    pub fn get_num_fvar_channels(&self) -> i32 { self.base.get_num_fvar_channels() }
...
mod tests {
    ...
    impl TopologyRefinerMesh for QuadMesh {
        fn scheme_type(&self)       -> SchemeType    { SchemeType::Catmark }
        fn scheme_options(&self)    -> SdcOptions    { SdcOptions::default() }
        fn num_faces(&self)         -> i32           { 1 }
        fn num_fvar_channels(&self) -> i32           { 0 }
        fn has_holes(&self)         -> bool          { false }
        fn is_face_hole(&self, _: Index) -> bool     { false }
        fn face_size(&self, _: Index) -> i32         { 4 }
        fn face_vertex_indices(&self, _: Index, out: &mut [Index]) -> i32 {
            out[..4].copy_from_slice(&[0, 1, 2, 3]);
            4
        }
        fn face_fvar_value_indices(&self, _: Index, _: i32, _: &mut [Index]) -> i32 { 0 }
        fn populate_corner_descriptor(
            &self, _: Index, _: i32, vd: &mut VertexDescriptor,
        ) -> i32 {
        ...
        fn corner_incident_vtx_indices(&self, _: Index, _: i32, out: &mut [Index]) -> i32 {
            let n = out.len();
            for (i, v) in out.iter_mut().enumerate() { *v = i as i32; }
            n as i32
        }
        fn corner_incident_fvar_indices(&self, _: Index, _: i32, _: i32, _: &mut [Index]) -> i32 { 0 }
    }
