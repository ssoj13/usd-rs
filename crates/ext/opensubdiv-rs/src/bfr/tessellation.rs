...
impl TessellationOptions {
    ...
    pub fn get_facet_size(&self) -> i32 {
        if self.facet_size == 4 { 4 } else { 3 }
    }
    /// Effective facet stride.
    pub fn get_facet_stride(&self) -> i32 {
        if self.facet_stride > 0 {
            self.facet_stride
        } else {
            self.get_facet_size()
        }
...
impl Tessellation {
    ...
    pub fn new(param: Parameterization, rates: &[i32], options: TessellationOptions) -> Self {
    ...
    pub fn get_rates(&self, rates: &mut [i32]) -> i32 {
        ...
        for i in 0..n_outer {
            rates[i] = self.outer_rates[i];
        }
        for i in 0..n_inner {
            rates[n as usize + i] = self.inner_rates[if i > 0 { 1 } else { 0 }];
        }
    ...
    #[inline]
    pub fn get_num_coords(&self) -> i32 {
        self.num_boundary_pts + self.num_interior_pts
    }
    /// Number of boundary coordinates.
    #[inline]
    pub fn get_num_boundary_coords(&self) -> i32 {
        self.num_boundary_pts
    }
    /// Number of interior coordinates.
    #[inline]
    pub fn get_num_interior_coords(&self) -> i32 {
        self.num_interior_pts
    }
    ...
    #[inline]
    pub fn get_num_facets(&self) -> i32 {
        self.num_facets
    }
    ...
    pub fn get_boundary_coords<R: Float>(&self, buf: &mut [R]) -> i32 {
        ...
        match self.param.get_type() {
            ...
            ParameterizationType::QuadSubFaces => {
                qsub_get_boundary_coords(self.param, &self.outer_rates, stride, buf)
            }
        }
    }
    ...
    pub fn get_interior_coords<R: Float>(&self, buf: &mut [R]) -> i32 {
        ...
        match self.param.get_type() {
            ...
            ParameterizationType::QuadSubFaces => {
                qsub_get_interior_coords(self.param, self.inner_rates[0], stride, buf)
            }
        }
    }
    ...
    pub fn get_edge_coords<R: Float>(&self, edge: i32, buf: &mut [R]) -> i32 {
        ...
        match self.param.get_type() {
            ...
            ParameterizationType::QuadSubFaces => {
                qsub_get_edge_coords(self.param, edge, edge_res, stride, buf)
            }
        }
    }
    ...
    fn init_rates(&mut self, given: &[i32]) -> i32 {
        ...
        if ng < n {
            ...
            for i in 0..n {
                ...
                if self.outer_rates[i] != self.outer_rates[0] {
            ...
            if ng > n {
                ...
                if self.inner_rates[0] != self.outer_rates[0] {
                    self.is_uniform = false;
                }
                if self.inner_rates[1] != self.outer_rates[0] {
                    self.is_uniform = false;
                }
            } else if is_quad {
    ...
    fn init_inventory_qpoly(&mut self, sum_outer: i32) {
        ...
        if self.is_uniform {
            ...
            if res > 1 {
                self.num_interior_pts = qsub_count_interior_coords(n, res);
                self.num_facets =
                    qsub_count_non_uniform_facets(n, &self.outer_rates, res, self.triangulate);
            } else {
                self.num_interior_pts = 1;
                self.num_facets = sum_outer;
                self.triangle_fan = true;
...
impl FacetStrip {
    ...
    fn connect_uniform_quads(
        ...
        if self.inner_edges > 0 {
            ...
            for i in 1..=self.inner_edges {
                if i > self.inner_edges / 2 {
                    cur_sign = -tri_sign;
                }
                let out_j = out_i + 1;
                let in_j = if i < self.inner_edges {
                    in_i + d_in
                } else {
                    self.inner_last
                };
    ...
    fn connect_non_uniform_facets(
        ...
        let all_inner_edges =
            self.inner_edges + 2 - self.split_first as i32 - self.split_last as i32;
        ...
        let dt_slope_max = if (dt_max / 2) < dt_min {
            dt_min - 1
        } else {
            dt_max / 2
        };
...
fn quad_count_segmented_facets(outer: &[i32], tri: bool) -> i32 {
    ...
    let t_res = if (outer[0] * outer[2]) == 1 {
        &outer[1..]
    } else {
        &outer[0..]
    };
...
fn quad_count_non_uniform_facets(outer: &[i32], inner: &[i32; 2], tri: bool) -> i32 {
    ...
    let mut nb = 0i32;
    nb += if uni_e[0] {
        inner_u + 1 + !uni_c[1] as i32
    } else {
        quad_count_non_uniform_edge_facets(outer[0], u)
    };
    nb += if uni_e[1] {
        inner_v + 1 + !uni_c[2] as i32
    } else {
        quad_count_non_uniform_edge_facets(outer[1], v)
    };
    nb += if uni_e[2] {
        inner_u + 1 + !uni_c[3] as i32
    } else {
        quad_count_non_uniform_edge_facets(outer[2], u)
    };
    nb += if uni_e[3] {
        inner_v + 1 + !uni_c[0] as i32
    } else {
        quad_count_non_uniform_edge_facets(outer[3], v)
    };
    n_interior + nb
}
...
fn quad_get_edge_coords<R: Float>(edge: i32, res: i32, stride: usize, buf: &mut [R]) -> i32 {
    ...
    match edge {
        0 => {
            append_v_isoline(buf, stride, n, t0, zero, dt);
        }
        1 => {
            append_u_isoline(buf, stride, n, one, t0, dt);
        }
        2 => {
            append_v_isoline(buf, stride, n, t1, one, -dt);
        }
        3 => {
            append_u_isoline(buf, stride, n, zero, t1, -dt);
        }
        _ => {}
    }
    n as i32
...
fn quad_get_interior_coords<R: Float>(inner: &[i32; 2], stride: usize, buf: &mut [R]) -> i32 {
    ...
    for _ in 0..n_int_rings {
        n += quad_get_interior_ring_coords(
            u_res,
            v_res,
            u,
            v,
...
fn quad_get_interior_ring_coords<R: Float>(
    u_res: i32,
    v_res: i32,
    u0: R,
    v0: R,
    ...
    if u_res > 0 && v_res > 0 {
        ...
        let v1 = R::one() - v0;
        let mut n = 0usize;
        append_v_isoline(&mut buf[n * stride..], stride, u_res as usize, u0, v0, du);
        n += u_res as usize;
        append_u_isoline(&mut buf[n * stride..], stride, v_res as usize, u1, v0, dv);
        n += v_res as usize;
        append_v_isoline(&mut buf[n * stride..], stride, u_res as usize, u1, v1, -du);
        n += u_res as usize;
        append_u_isoline(&mut buf[n * stride..], stride, v_res as usize, u0, v1, -dv);
        n += v_res as usize;
        n as i32
    } else if u_res > 0 {
        ...
        // centre:
        buf[0] = R::from(0.5).unwrap();
        buf[1] = R::from(0.5).unwrap();
        1
    }
}
...
fn quad_get_uniform_facets(
    ...
    let mut cur_res = res;
    for _ in 0..n_rings {
        let n = quad_get_interior_ring_facets(
            cur_res,
            cur_res,
            coord0,
            tri,
...
fn quad_get_non_uniform_facets(
    outer: &[i32],
    inner: &[i32; 2],
    n_boundary: usize,
    tri: bool,
...
fn quad_get_single_strip_facets(
    u_res: i32,
    v_res: i32,
    coord0: i32,
    tri: bool,
...
fn quad_get_interior_ring_facets(
    u_res: i32,
    v_res: i32,
    coord0: i32,
    tri: bool,
...
fn quad_get_boundary_ring_facets(
    outer: &[i32],
    u_res: i32,
    v_res: i32,
    n_boundary: usize,
    ...
    if uni_e[0] {
        strip.split_first = !uni_c[0];
        strip.split_last = !uni_c[1];
        nf += strip.connect_uniform_quads(buf, nf as usize * fstride, fsize, fstride);
    } else {
    ...
    if uni_e[1] {
        strip.split_first = !uni_c[1];
        strip.split_last = !uni_c[2];
        nf += strip.connect_uniform_quads(buf, nf as usize * fstride, fsize, fstride);
    } else {
        strip.split_first = true;
        strip.split_last = true;
    ...
    if uni_e[2] {
        strip.split_first = !uni_c[2];
        strip.split_last = !uni_c[3];
        nf += strip.connect_uniform_quads(buf, nf as usize * fstride, fsize, fstride);
    } else {
        strip.split_first = true;
        strip.split_last = true;
    ...
    if uni_e[3] {
        strip.split_first = !uni_c[3];
        strip.split_last = !uni_c[0];
        nf += strip.connect_uniform_quads(buf, nf as usize * fstride, fsize, fstride);
    } else {
        strip.split_first = true;
        strip.split_last = true;
...
fn tri_count_uniform_facets(res: i32) -> i32 {
    res * res
}
fn tri_count_uniform_coords(res: i32) -> i32 {
    res * (res + 1) / 2
}
fn tri_count_interior_coords(res: i32) -> i32 {
    tri_count_uniform_coords(res - 2)
}

fn tri_count_non_uniform_facets(outer: &[i32], inner: i32) -> i32 {
    let inner_edges = inner - 3;
    let n_interior = if inner_edges > 0 {
        tri_count_uniform_facets(inner_edges)
    } else {
        0
    };
    let n_boundary = (inner_edges + outer[0]) + (inner_edges + outer[1]) + (inner_edges + outer[2]);
...
fn tri_get_edge_coords<R: Float>(edge: i32, res: i32, stride: usize, buf: &mut [R]) -> i32 {
    ...
    match edge {
        ...
        1 => {
            append_uv_line(buf, stride, n, t1, t0, -dt, dt);
        }
        2 => {
            append_u_isoline(buf, stride, n, zero, t1, -dt);
        }
...
fn tri_get_interior_coords<R: Float>(res: i32, stride: usize, buf: &mut [R]) -> i32 {
    ...
    for _ in 0..n_rings {
        if ring_res == 0 {
            ...
            n += tri_get_interior_ring_coords(ring_res, u, v, dt, stride, &mut buf[n * stride..])
                as usize;
        }
        ring_res -= 3;
        u = u + dt;
...
fn tri_get_interior_ring_coords<R: Float>(
    res: i32,
    u0: R,
    v0: R,
    dt: R,
    ...
    let v1 = one - v0 * two;
    let mut n = 0usize;
    append_v_isoline(&mut buf[n * stride..], stride, res as usize, u0, v0, dt);
    n += res as usize;
    append_uv_line(
        &mut buf[n * stride..],
        stride,
    ...
    n += res as usize;
    append_u_isoline(&mut buf[n * stride..], stride, res as usize, u0, v1, -dt);
    n += res as usize;
    n as i32
}
...
fn tri_get_uniform_facets(res: i32, fsize: usize, fstride: usize, buf: &mut [i32]) -> i32 {
    ...
    let mut cur = res;
    for _ in 0..n_rings {
        let n = tri_get_interior_ring_facets(
            cur,
            coord0,
            fsize,
            fstride,
...
fn tri_get_interior_ring_facets(
    res: i32,
    coord0: i32,
    fsize: usize,
    fstride: usize,
...
fn tri_get_boundary_ring_facets(
    outer: &[i32],
    inner: i32,
    n_boundary: usize,
    fsize: usize,
...
fn qsub_get_boundary_coords<R: Float>(
    param: Parameterization,
    outer: &[i32],
    stride: usize,
    buf: &mut [R],
    ...
    for i in 0..n {
        ...
        off += qsub_get_ring_edge_coords(
            ...
            false,
            R::zero(),
            R::one() / R::from(outer[i as usize]).unwrap(),
            stride,
            &mut buf[off * stride..],
        ) as usize;
    }
...
fn qsub_get_edge_coords<R: Float>(
    param: Parameterization,
    edge: i32,
    res: i32,
    stride: usize,
    ...
    qsub_get_ring_edge_coords(
        ...
        false,
        R::zero(),
        R::one() / R::from(res).unwrap(),
        stride,
        buf,
    )
}

fn qsub_get_ring_edge_coords<R: Float>(
    param: Parameterization,
    edge: i32,
    res: i32,
    inc_first: bool,
...
fn qsub_get_interior_coords<R: Float>(
    param: Parameterization,
    res: i32,
    stride: usize,
    buf: &mut [R],
    ...
    for _ in 0..n_rings {
        if ring_res == 0 {
            buf[n * stride] = R::from(0.5).unwrap();
            buf[n * stride + 1] = R::from(0.5).unwrap();
            n += 1;
        } else if ring_res == 1 {
            // centre ring coords:
            ...
            n += qsub_get_interior_ring_coords(
                param,
                ring_res,
                t,
                dt,
...
fn qsub_get_interior_ring_coords<R: Float>(
    param: Parameterization,
    res: i32,
    t_origin: R,
    dt: R,
...
fn qsub_get_uniform_facets(
    ...
    let mut cur = res;
    for _ in 0..n_rings {
        let cnt = qsub_get_interior_ring_facets(
            n,
            cur,
            coord0,
            tri,
...
fn qsub_get_non_uniform_facets(
    n: i32,
    outer: &[i32],
    inner: i32,
    n_boundary: usize,
    ...
    let mut nf =
        qsub_get_boundary_ring_facets(n, outer, inner, n_boundary, tri, fsize, fstride, buf);
...
fn qsub_get_interior_ring_facets(
    n: i32,
    res: i32,
    coord0: i32,
    tri: bool,
...
fn qsub_get_boundary_ring_facets(
    n: i32,
    outer: &[i32],
    inner: i32,
    n_boundary: usize,
...
mod tests {
    ...
    fn quad_uniform_rate1_single_facet() {
        let t = Tessellation::new_uniform(
            quad_param(),
            1,
