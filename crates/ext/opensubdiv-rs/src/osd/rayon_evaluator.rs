...
use crate::osd::{
    ...
    patch_basis::{
        OsdPatchParam, osd_evaluate_patch_basis,
...
impl RayonEvaluator {
    ...
    pub fn eval_stencils_raw(
        ...
        let base_offset = if start > 0 { offsets[start as usize] as usize } else { 0 };
        ...
        dst_region
            ...
            .for_each(|(chunk, &(off, sz))| {
                ...
                for v in chunk[..len].iter_mut() { *v = 0.0; }
                for j in 0..sz {
            ...
            .for_each(|(i, &(off, sz))| {
                ...
                unsafe {
                    std::ptr::copy_nonoverlapping(
                        acc.as_ptr(), dst_ptr.add(dst_base + i * stride_p), len);
                    std::ptr::copy_nonoverlapping(
                        acc_du.as_ptr(), du_ptr.add(du_base + i * stride_u), len);
                    std::ptr::copy_nonoverlapping(
                        acc_dv.as_ptr(), dv_ptr.add(dv_base + i * stride_v), len);
                }
            });
        true
    }
    ...
    pub fn eval_stencils_d2_raw(
        ...
        stencil_offsets
            ...
            .for_each(|(i, &(off, sz))| {
                ...
                unsafe {
                    std::ptr::copy_nonoverlapping(a.as_ptr(),     dst_ptr.add(base_p  + i * stride_p),  len);
                    std::ptr::copy_nonoverlapping(a_du.as_ptr(),  du_ptr .add(base_u  + i * stride_u),  len);
                    std::ptr::copy_nonoverlapping(a_dv.as_ptr(),  dv_ptr .add(base_v  + i * stride_v),  len);
                    std::ptr::copy_nonoverlapping(a_duu.as_ptr(), duu_ptr.add(base_uu + i * stride_uu), len);
                    std::ptr::copy_nonoverlapping(a_duv.as_ptr(), duv_ptr.add(base_uv + i * stride_uv), len);
                    std::ptr::copy_nonoverlapping(a_dvv.as_ptr(), dvv_ptr.add(base_vv + i * stride_vv), len);
                }
            });
        true
    }
    ...
    pub fn eval_patches_d2_raw(
        ...
        for (i, (r, ru, rv, ruu, ruv, rvv)) in results.iter().enumerate() {
            ...
            dst[po..po + len].copy_from_slice(r);
            if let Some(ref mut b) = du_buf  { let o = du_base  + i * du_stride;  b[o..o + len].copy_from_slice(ru);  }
            if let Some(ref mut b) = dv_buf  { let o = dv_base  + i * dv_stride;  b[o..o + len].copy_from_slice(rv);  }
            if let Some(ref mut b) = duu_buf { let o = duu_base + i * duu_stride; b[o..o + len].copy_from_slice(ruu); }
            if let Some(ref mut b) = duv_buf { let o = duv_base + i * duv_stride; b[o..o + len].copy_from_slice(ruv); }
            if let Some(ref mut b) = dvv_buf { let o = dvv_base + i * dvv_stride; b[o..o + len].copy_from_slice(rvv); }
        }
        true
    }
