...
impl CpuEvaluator {
    ...
    pub fn eval_patches_d1_raw(
        ...
        for coord in patch_coords {
            ...
            for v in dst[dst_off..dst_off + len].iter_mut() {
                *v = 0.0;
            }
            if let Some(ref mut bu) = du_buf {
                for v in bu[du_off..du_off + len].iter_mut() {
                    *v = 0.0;
                }
            }
            if let Some(ref mut bv) = dv_buf {
                for v in bv[dv_off..dv_off + len].iter_mut() {
                    *v = 0.0;
                }
            }
            ...
            for j in 0..n_pts as usize {
                ...
                if let Some(ref mut bu) = du_buf {
                    for k in 0..len {
                        bu[du_off + k] += src_s[src_off + k] * wds[j];
                    }
                }
                if let Some(ref mut bv) = dv_buf {
                    for k in 0..len {
                        bv[dv_off + k] += src_s[src_off + k] * wdt[j];
                    }
                }
            }
            dst_off += dst_stride;
    ...
    pub fn eval_patches_d2_raw(
        ...
        for coord in patch_coords {
            ...
            if let Some(ref mut b) = du_buf {
                for v in b[du_off..du_off + len].iter_mut() {
                    *v = 0.0;
                }
            }
            if let Some(ref mut b) = dv_buf {
                for v in b[dv_off..dv_off + len].iter_mut() {
                    *v = 0.0;
                }
            }
            if let Some(ref mut b) = duu_buf {
                for v in b[duu_off..duu_off + len].iter_mut() {
                    *v = 0.0;
                }
            }
            if let Some(ref mut b) = duv_buf {
                for v in b[duv_off..duv_off + len].iter_mut() {
                    *v = 0.0;
                }
            }
            if let Some(ref mut b) = dvv_buf {
                for v in b[dvv_off..dvv_off + len].iter_mut() {
                    *v = 0.0;
                }
            }
            ...
            for j in 0..n_pts as usize {
                ...
                if let Some(ref mut b) = du_buf {
                    for k in 0..len {
                        b[du_off + k] += src_s[src_off + k] * wds[j];
                    }
                }
                if let Some(ref mut b) = dv_buf {
                    for k in 0..len {
                        b[dv_off + k] += src_s[src_off + k] * wdt[j];
                    }
                }
                if let Some(ref mut b) = duu_buf {
                    for k in 0..len {
                        b[duu_off + k] += src_s[src_off + k] * wdss[j];
                    }
                }
                if let Some(ref mut b) = duv_buf {
                    for k in 0..len {
                        b[duv_off + k] += src_s[src_off + k] * wdst[j];
                    }
                }
                if let Some(ref mut b) = dvv_buf {
                    for k in 0..len {
                        b[dvv_off + k] += src_s[src_off + k] * wdtt[j];
                    }
                }
            }
            dst_off += dst_stride;
...
mod tests {
    ...
    fn eval_stencils_empty_range() {
        ...
        let ok = CpuEvaluator::eval_stencils(&src, &d, &mut dst, &d, &[], &[], &[], &[], 0, 0);
        assert!(ok);
    }
