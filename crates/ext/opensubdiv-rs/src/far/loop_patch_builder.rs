...
#[inline]
fn assign_row<R: Copy + Default>(m: &mut SparseMatrix<R>, row: i32, idx: &[i32], w: &[R]) {
...
impl<'r> LoopPatchBuilder<'r> {
    ...
    fn convert_source_patch_f32(
        ...
        sp: &SourcePatch,
        pt: PatchType,
        m: &mut SparseMatrix<f32>,
    ) -> i32 {
        match pt {
            PatchType::Loop => {
                convert_to_loop_f32(sp, m);
                m.get_num_rows()
            }
            PatchType::Triangles => {
                convert_to_linear_f32(sp, m);
                m.get_num_rows()
            }
            PatchType::GregoryTriangle => {
                convert_to_gregory_f32(sp, m);
                m.get_num_rows()
            }
            _ => 0,
        }
    ...
    fn convert_source_patch_f64(
        ...
        sp: &SourcePatch,
        pt: PatchType,
        m: &mut SparseMatrix<f64>,
    ) -> i32 {
        match pt {
            PatchType::Loop => {
                convert_to_loop_f64(sp, m);
                m.get_num_rows()
            }
            PatchType::Triangles => {
                convert_to_linear_f64(sp, m);
                m.get_num_rows()
            }
            PatchType::GregoryTriangle => {
                convert_to_gregory_f64(sp, m);
                m.get_num_rows()
            }
            _ => 0,
        }
...
fn loop_limits_interior<R: Copy + Default + num_traits::Float>(
    ...
    for i in 1..rs {
        pw[i] = R::from(ew).unwrap_or_default();
    }
    ...
    if let (Some(epw), Some(emw)) = (ep, em) {
        ...
        t2[0] = vw;
        for i in 1..rs {
            t2[i] = ew + ((i - 1) as f64 * theta).cos() * ts;
        }
        ...
        for i in 0..n1e {
            epw[1 + i] = R::from(t2[1 + n2e + i]).unwrap_or_default();
        }
        for i in 0..n2e {
            epw[1 + n1e + i] = R::from(t2[1 + i]).unwrap_or_default();
        }
        ...
        for i in 0..n1m {
            emw[1 + i] = R::from(t2[1 + n2m + i]).unwrap_or_default();
        }
        for i in 0..n2m {
            emw[1 + n1m + i] = R::from(t2[1 + i]).unwrap_or_default();
        }
    }
}
...
fn loop_limits_boundary<R: Copy + Default + num_traits::Float>(
    ...
    if let (Some(epw), Some(emw)) = (ep, em) {
        ...
        for i in 1..valence {
            t2w[i] = (fa * i as f64).sin() / (n - 1.0);
        }

        // ep
        if face_in == 0 {
            for v in epw.iter_mut() {
                *v = R::zero();
            }
            ...
            for i in 0..rs {
                epw[i] = R::from(t2w[i] * sa / 24.0).unwrap_or_default();
            }
            epw[0] = R::from(epw[0].to_f64().unwrap_or(0.0) + 2.0 / 3.0).unwrap_or_default();
            epw[1] =
                R::from(epw[1].to_f64().unwrap_or(0.0) + 1.0 / 6.0 + ca / 6.0).unwrap_or_default();
            epw[valence] = R::from(epw[valence].to_f64().unwrap_or(0.0) + 1.0 / 6.0 - ca / 6.0)
                .unwrap_or_default();
        }

        // em
        if face_in == valence - 1 {
            for v in emw.iter_mut() {
                *v = R::zero();
            }
            ...
            let (ca, sa) = (a.cos(), a.sin());
            for i in 0..rs {
                emw[i] = R::from(t2w[i] * sa / 24.0).unwrap_or_default();
            }
            emw[0] = R::from(emw[0].to_f64().unwrap_or(0.0) + 2.0 / 3.0).unwrap_or_default();
            emw[1] =
                R::from(emw[1].to_f64().unwrap_or(0.0) + 1.0 / 6.0 + ca / 6.0).unwrap_or_default();
            emw[valence] = R::from(emw[valence].to_f64().unwrap_or(0.0) + 1.0 / 6.0 - ca / 6.0)
                .unwrap_or_default();
        }
    }
}
...
fn resize_matrix<R: Copy + Default>(
    ...
    m.resize(nr, nc, ne);
    for i in 0..nr {
        m.set_row_size(i, sizes[i as usize]);
    }
}

fn add_row_to_full<R>(full: &mut [R], mat: &SparseMatrix<R>, row: i32, s: R)
where
    R: Copy + Default + std::ops::AddAssign + std::ops::Mul<Output = R>,
{
...
fn combine_rows_in_full<R>(
    dst: &mut SparseMatrix<R>,
    dst_row: i32,
    src: &SparseMatrix<R>,
    src_rows: &[i32],
    src_w: &[R],
) where
    R: Copy + Default + std::ops::AddAssign + std::ops::Mul<Output = R> + PartialEq,
{
    ...
    for (&ri, &w) in src_rows.iter().zip(src_w.iter()) {
        if w != zero {
            add_row_to_full(&mut full, src, ri, w);
        }
    }
...
where
    R: Copy + Default + num_traits::Float + std::ops::AddAssign,
{
    ...
    R: Copy + Default + num_traits::Float + std::ops::AddAssign,
{
    ...
    for ci in 0..3 {
        ...
        rsz[ci] = if c.sharp {
            ...
            1 + sp.get_corner_ring_size(ci)
        };
        ne += rsz[ci];
    }
    m.resize(3, nsrc, ne);
    for i in 0..3 {
        m.set_row_size(i as i32, rsz[i]);
    }
    ...
    if has_val2 {
        remove_valence2_duplicates(m);
    }
}

fn convert_to_linear_f32(sp: &SourcePatch, m: &mut SparseMatrix<f32>) {
    convert_to_linear_generic(sp, m);
}
fn convert_to_linear_f64(sp: &SourcePatch, m: &mut SparseMatrix<f64>) {
    convert_to_linear_generic(sp, m);
}
...
fn convert_to_loop_f64(sp: &SourcePatch, m: &mut SparseMatrix<f64>) {
    ...
    init_full_matrix(m, 12, nc);
    for i in 0..12i32 {
        let rw: Vec<f64> = GREGORY_TO_LOOP[i as usize]
            .iter()
            .map(|&v| v as f64)
            .collect();
        combine_rows_in_full(m, i, &g, &G_ROW_INDICES, &rw);
    ...
    R: Copy + Default + num_traits::Float + std::ops::AddAssign,
{
    ...
    fn init(&mut self, sp: &SourcePatch) {
        ...
        for ci in 0..3usize {
            ...
            c.cos_fa = if c.is_reg {
                0.5
            } else if c.is_bnd {
                (PI / c.nf as f64).cos()
            } else {
                (2.0 * PI / c.nf as f64).cos()
            };
        ...
        for ci in 0..3usize {
            ...
            self.c[ci].fp_copy = false;
            self.c[ci].fm_copy = false;
            self.c[ci].ep_bnd = false;
            self.c[ci].em_bnd = false;
            ...
            if self.c[ci].is_bnd {
                ...
                if nf > 1 {
                    if self.c[ci].ep_bnd {
                        self.c[ci].fp_reg = self.c[ci].fm_reg;
                        self.c[ci].fp_copy = !self.c[ci].fp_reg;
                    }
                    if self.c[ci].em_bnd {
                        self.c[ci].fm_reg = self.c[ci].fp_reg;
                        self.c[ci].fm_copy = !self.c[ci].fm_reg;
                    }
                } else {
                    self.c[ci].fp_reg = true;
                    self.c[ci].fm_reg = true;
                }
            }
        }
        ...
        for ci in 0..3usize {
            ...
            if c.is_reg {
                if !c.is_bnd {
                    ...
                    s[0] = 3;
                    s[1] = if c.ep_bnd { 3 } else { 5 };
                    s[2] = if c.em_bnd { 3 } else { 5 };
                }
            } else if c.is_sharp {
                s[0] = 1;
                s[1] = 2;
                ...
                let r = 1 + c.val;
                s[0] = 3;
                s[1] = if c.ep_bnd { 3 } else { r };
                s[2] = if c.em_bnd { 3 } else { r };
            } else {
                s[0] = 3;
                s[1] = 3;
                s[2] = 3;
            ...
            s[3] = if !c.fp_reg {
                self.fp_size(ci, if c.fp_copy { cp } else { cn })
            } else {
                bfp
            };
            s[4] = if !c.fm_reg {
                self.fp_size(ci, if c.fm_copy { cn } else { cp })
            } else {
                bfp
            };
            ne += s[3] + s[4];
            ...
            let cn_c = &self.c[cn];
            sz[15 + ci] = if c.ep_bnd && cn_c.em_bnd {
                2
            } else if c.is_reg && cn_c.is_reg && (c.ep_bnd == cn_c.em_bnd) {
                4
            } else {
                self.fp_size(ci, cn)
            };
    ...
    fn assign_reg_edge(&self, ci: usize, m: &mut SparseMatrix<R>) {
        ...
        if !c.is_bnd {
            ...
            for i in 0..6 {
                ix[1 + i] = ring[(fi + i) % 6];
                wx[1 + i] = es * R::from(ew[i]).unwrap_or_default();
            }
            ...
            if c.ep_bnd {
                ...
                let eo = if c.em_bnd { 3 } else { 0 };
                let ez = if c.em_bnd { 0 } else { 3 };
                assign_row(
                    m,
                    b + 1,
                    &[ci as i32, ring[1], ring[2], ring[eo], ring[ez]],
                    &[h, s6, s6, s6, z],
                );
            }
            ...
            if c.em_bnd {
                ...
                let eo = if c.ep_bnd { 0 } else { 3 };
                let ez = if c.ep_bnd { 3 } else { 0 };
                assign_row(
                    m,
                    b + 2,
                    &[ci as i32, ring[1], ring[2], ring[eo], ring[ez]],
    ...
    fn compute_irreg_edge(&self, ci: usize, m: &mut SparseMatrix<R>, wb: &mut Vec<R>) {
        ...
        if c.is_sharp {
            ...
            assign_row(
                m,
                b,
                &[ci as i32, ((ci + 1) % 3) as i32, ((ci + 2) % 3) as i32],
                &[f46, f16, f16],
            );
            assign_row(
                m,
            ...
            assign_row(
                m,
                b + 2,
                &[ci as i32, ((ci + 2) % 3) as i32, ((ci + 1) % 3) as i32],
                &[f23, f13, z],
            );
        }
    }
    ...
    fn compute_irreg_int_edge(&self, ci: usize, m: &mut SparseMatrix<R>, wb: &mut Vec<R>) {
        let c = &self.c[ci];
        let val = c.val as usize;
        let ww = val + 1;
        let b = (5 * ci) as i32;
        let ring = c.ring.clone();
        wb.resize(3 * ww, R::zero());
        let (pb, rest) = wb.split_at_mut(ww);
        let (ep, em) = rest.split_at_mut(ww);
        loop_limits_interior::<R>(val, c.fi as usize, pb, Some(ep), Some(em));
        let pw = pb.to_vec();
        let epw = ep.to_vec();
        let emw = em.to_vec();
        let mut ix = vec![ci as i32];
        for r in &ring {
            ix.push(*r);
        }
        ...
        for fisfm in 0..2usize {
            ...
            if c.is_corner {
                assign_row(
                    m,
                    fr,
                    &[ci as i32, cn as i32, cp as i32],
                    &[
                        R::from(0.5).unwrap_or_default(),
                        R::from(0.25).unwrap_or_default(),
                        R::from(0.25).unwrap_or_default(),
                    ],
                );
            } else if c.ep_bnd {
                assign_row(
                    m,
                    fr,
                    &[ci as i32, ring[0], ring[1], ring[2]],
                    &[
                        R::from(11.0 / 24.0).unwrap_or_default(),
                        R::from(7.0 / 24.0).unwrap_or_default(),
                        R::from(5.0 / 24.0).unwrap_or_default(),
                ...
                assign_row(
                    m,
                    fr,
                    &[ci as i32, ring[3], ring[2], ring[1]],
                    &[
                        R::from(11.0 / 24.0).unwrap_or_default(),
                        R::from(7.0 / 24.0).unwrap_or_default(),
                        R::from(5.0 / 24.0).unwrap_or_default(),
                ...
                assign_row(
                    m,
                    fr,
                    &[ci as i32, cp as i32, cn as i32, ring[ep], ring[en]],
                    &[
                        R::from(10.0 / 24.0).unwrap_or_default(),
                        R::from(0.25).unwrap_or_default(),
                        R::from(0.25).unwrap_or_default(),
                        R::from(1.0 / 24.0).unwrap_or_default(),
                        R::from(1.0 / 24.0).unwrap_or_default(),
    ...
    fn compute_irreg_face(
        ...
        if c.fp_copy {
            let fc = m.get_row_columns(b + 4).to_vec();
            let fw = m.get_row_elements(b + 4).to_vec();
            assign_row(m, b + 3, &fc, &fw);
        }
        if c.fm_copy {
            let fc = m.get_row_columns(b + 3).to_vec();
            let fw = m.get_row_elements(b + 3).to_vec();
    ...
    fn one_face_pt(
        ...
        let cosn = self.c[cn].cos_fa;
        let cosf = self.c[cf].cos_fa;
        let ring = self.c[cn].ring.clone();
        let val = self.c[cn].val as usize;
        ...
        let ip = (en + val - 1) % val;
        let inx = (en + 1) % val;
        let rp = ring[ip] as usize;
        let rn = ring[inx] as usize;
        rw[rp] += -sg * rs;
        cm[rp] = 1 + rp as i32;
        rw[rn] += sg * rs;
        cm[rn] = 1 + rn as i32;
        ...
        let mut oi = Vec::with_capacity(dsz);
        let mut ow = Vec::with_capacity(dsz);
        let mut cnt = 0;
        for i in 0..n {
            if cm[i] != 0 {
                oi.push(cm[i] - 1);
    ...
    fn assign_reg_mid_edge(&self, ei: usize, m: &mut SparseMatrix<R>) {
        ...
        if c.ep_bnd {
            ...
            assign_row(
                m,
                mr,
                &[ei as i32, cn as i32, ((ei + 2) % 3) as i32, ov],
                &[t3, t3, s6, s6],
            );
        }
    }
    ...
    fn compute_irreg_mid_edge(
        ...
        let e0p = (5 * ei + 1) as i32;
        let e1m = (5 * ((ei + 1) % 3) + 2) as i32;
        let mr = (15 + ei) as i32;
        let h = R::from(0.5).unwrap_or_default();
    ...
    fn promote_quartic(&self, m: &mut SparseMatrix<R>, rw: &mut Vec<R>, cm: &mut Vec<i32>) {
        ...
        for ci in 0..3usize {
            ...
            for ep in 0..2usize {
                ...
                let tbl: Option<&[f64]> = if ob && !c.is_sharp {
                    Some(&bw)
                } else if c.is_reg {
                    if c.is_bnd { Some(&rbw) } else { Some(&riw) }
                } else {
                    None
                };
                if let Some(t) = tbl {
                    ...
                    for (i, &w) in t.iter().enumerate() {
                        if i < el.len() {
                            el[i] = ov24 * R::from(w).unwrap_or_default();
                        }
                    }
                } else {
                    ...
                    let (oi, ow) =
                        combine_two_rows(m, pr, qtr, er, tqr, self.nsrc as usize, rw, cm);
    ...
    fn convert(&self, m: &mut SparseMatrix<R>) {
        if self.isolated {
            self.size_isolated(m);
        } else {
            self.size_unisolated(m);
        }
        ...
        for ci in 0..3 {
            if self.c[ci].is_reg {
                self.assign_reg_edge(ci, m);
            } else {
                self.compute_irreg_edge(ci, m, &mut rw);
            }
        }
        for ci in 0..3 {
            let c = &self.c[ci];
            if c.fp_reg || c.fm_reg {
                self.assign_reg_face(ci, m);
            }
            if !c.fp_reg || !c.fm_reg {
                self.compute_irreg_face(ci, m, &mut rw, &mut cm);
            }
        }
        for ei in 0..3 {
            let c0 = &self.c[ei];
            let c1 = &self.c[(ei + 1) % 3];
            let bnd = c0.ep_bnd && c1.em_bnd;
            let drt = c0.ep_bnd != c1.em_bnd;
            if bnd || (c0.is_reg && c1.is_reg && !drt) {
                self.assign_reg_mid_edge(ei, m);
            } else {
                self.compute_irreg_mid_edge(ei, m, &mut rw, &mut cm);
            }
        }
...
fn convert_to_gregory_f32(sp: &SourcePatch, m: &mut SparseMatrix<f32>) {
    convert_to_gregory_generic(sp, m);
}
fn convert_to_gregory_f64(sp: &SourcePatch, m: &mut SparseMatrix<f64>) {
    convert_to_gregory_generic(sp, m);
}
...
where
    R: std::ops::AddAssign,
{
    let nr = m.get_num_rows();
    let nc = m.get_num_columns() as usize;
    ...
    for row in 0..nr {
        ...
        let mut oc = vec![0i32; ds];
        let mut ow = vec![R::default(); ds];

        if dups > 0 {
            let mut pos = [usize::MAX; 4];
            let mut oi = 0;
            for i in 0..ss {
                let si_ = si[i] as usize;
                let sw_ = sw[i];
                if si_ < 4 {
                    if pos[si_] != usize::MAX {
                        ow[pos[si_]] += sw_;
                        continue;
                    }
                    pos[si_] = oi;
            ...
            oc.copy_from_slice(&si);
            ow.copy_from_slice(&sw);
        }
...
pub fn convert_loop(sp: &SourcePatch, pt: PatchType, m: &mut SparseMatrix<f32>) -> i32 {
    match pt {
        PatchType::Loop => {
            convert_to_loop_f32(sp, m);
            m.get_num_rows()
...
mod tests {
    ...
    fn loop_patch_type_from_basis_map() {
        ...
        let pb = LoopPatchBuilder::new(&r, PatchBuilderOptions::default());
        assert_eq!(
            pb.patch_type_from_basis(BasisType::Regular),
            PatchType::Loop
        );
        assert_eq!(
            pb.patch_type_from_basis(BasisType::Gregory),
            PatchType::GregoryTriangle
        );
        assert_eq!(
            pb.patch_type_from_basis(BasisType::Linear),
            PatchType::Triangles
        );
    }
    ...
    fn convert_linear_3_rows() {
        ...
        assert_eq!(
            pb.convert_to_patch_type_f32(&sp, PatchType::Triangles, &mut m),
            3
        );
    }
    ...
    fn convert_gregory_18_rows() {
        ...
        let mut m = SparseMatrix::new();
        assert_eq!(
            pb.convert_to_patch_type_f32(&sp, PatchType::GregoryTriangle, &mut m),
            18
        );
    }
    ...
    fn convert_loop_12_rows() {
        ...
        let mut m = SparseMatrix::new();
        assert_eq!(
            pb.convert_to_patch_type_f32(&sp, PatchType::Loop, &mut m),
            12
        );
    }
    ...
    fn gregory_converter_boundary() {
        let mut sp = SourcePatch::new();
        sp.corners[0].num_faces = 3;
        sp.corners[0].patch_face = 0;
        sp.corners[0].boundary = true;
        sp.corners[0].sharp = false;
        for i in 1..3 {
            sp.corners[i].num_faces = 6;
            sp.corners[i].patch_face = 0;
            sp.corners[i].boundary = false;
