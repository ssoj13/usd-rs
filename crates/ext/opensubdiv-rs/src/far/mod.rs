...
pub use topology_descriptor::{TopologyDescriptor, FVarChannel};
pub use topology_refiner::{TopologyRefiner, UniformOptions, AdaptiveOptions};
pub use topology_level::TopologyLevel;
pub use topology_refiner_factory::{TopologyRefinerFactory, FactoryOptions,
                                   TopologyDescriptorFactory};
pub use stencil_table::{StencilTable, LimitStencilTable};
pub use patch_descriptor::{PatchDescriptor, PatchType};
pub use patch_param::PatchParam;
pub use patch_table::{PatchTable, PatchHandle};
pub use patch_map::PatchMap;
pub use patch_table_factory::{PatchTableFactory, Options as PatchTableFactoryOptions, EndCapType};
...
pub fn evaluate_patch_basis(
    ...
    osd_eval_f32(patch_type_id, &osd_param, s, t, wp_slice, wds, wdt, wdss, wdst, wdtt)
}
...
pub fn evaluate_patch_basis_f64(
    ...
    let npts = osd_eval_f32(
        patch_type_id, &osd_param, s as f32, t as f32,
        &mut wp32,
        if wds.is_some()  { Some(&mut wds32)  } else { None },
        if wdt.is_some()  { Some(&mut wdt32)  } else { None },
        if wdss.is_some() { Some(&mut wdss32) } else { None },
        if wdst.is_some() { Some(&mut wdst32) } else { None },
        if wdtt.is_some() { Some(&mut wdtt32) } else { None },
    ) as usize;
    ...
    if let Some(ref mut out) = wds {
        for (a, &b) in out[..npts].iter_mut().zip(wds32[..npts].iter()) { *a = b as f64; }
    }
    if let Some(ref mut out) = wdt {
        for (a, &b) in out[..npts].iter_mut().zip(wdt32[..npts].iter()) { *a = b as f64; }
    }
    if let Some(ref mut out) = wdss {
        for (a, &b) in out[..npts].iter_mut().zip(wdss32[..npts].iter()) { *a = b as f64; }
    }
    if let Some(ref mut out) = wdst {
        for (a, &b) in out[..npts].iter_mut().zip(wdst32[..npts].iter()) { *a = b as f64; }
    }
    if let Some(ref mut out) = wdtt {
        for (a, &b) in out[..npts].iter_mut().zip(wdtt32[..npts].iter()) { *a = b as f64; }
    }
