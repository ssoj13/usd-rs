...
use crate::sdc::{
    Options,
    types::{SchemeType, Split, SchemeTypeTraits},
    crease::Rule,
};
use crate::vtr::{
    Level, Refinement, RefinementOptions,
    QuadRefinement, TriRefinement, SparseSelector, VTag,
};
use super::types::{Index, ConstIndexArray};
...
impl AdaptiveOptions {
    ...
    pub fn set_isolation_level(&mut self, level: u32) { self.isolation_level = level & 0xf; }
    pub fn set_secondary_level(&mut self, level: u32) { self.secondary_level = level & 0xf; }
}
...
impl TopologyRefiner {
    ...
    pub fn get_scheme_type(&self)    -> SchemeType { self.subdiv_type }
    pub fn get_scheme_options(&self) -> Options    { self.subdiv_options }
    pub fn is_uniform(&self)         -> bool       { self.is_uniform }
    pub fn get_num_levels(&self)     -> i32        { self.levels.len() as i32 }
    pub fn get_max_level(&self)      -> i32        { self.max_level as i32 }
    pub fn get_max_valence(&self)    -> i32        { self.max_valence }
    pub fn has_holes(&self)          -> bool       { self.has_holes }
    pub fn get_num_vertices_total(&self)      -> i32 { self.total_vertices }
    pub fn get_num_edges_total(&self)         -> i32 { self.total_edges }
    pub fn get_num_faces_total(&self)         -> i32 { self.total_faces }
    pub fn get_num_face_vertices_total(&self) -> i32 { self.total_face_vertices }
    ...
    pub fn get_uniform_options(&self)  -> UniformOptions  { self.uniform_options }
    pub fn get_adaptive_options(&self) -> AdaptiveOptions { self.adaptive_options }
    ...
    pub fn get_fvar_linear_interpolation(&self, channel: i32)
        -> crate::sdc::options::FVarLinearInterpolation
    {
        self.levels[0].get_fvar_options(channel).get_fvar_linear_interpolation()
    }
    pub fn get_num_fvar_values_total(&self, channel: i32) -> i32 {
        self.levels.iter().map(|l| l.get_num_fvar_values(channel)).sum()
    }
    ...
    pub fn refine_uniform(&mut self, options: UniformOptions) {
        ...
        for i in 1..=(options.refinement_level as usize) {
            ...
            let refinement = match split_type {
                Split::ToQuads => {
                    let mut qr = unsafe {
                        QuadRefinement::new(parent_ptr, child_ptr, self.subdiv_options)
                    };
                    qr.refine(ref_opts);
                    Box::new(qr.0)
                }
                _ => {
                    let mut tr = unsafe {
                        TriRefinement::new(parent_ptr, child_ptr, self.subdiv_options)
                    };
                    tr.refine(ref_opts);
                    Box::new(tr.0)
                }
    ...
    pub fn refine_adaptive(&mut self, options: AdaptiveOptions, selected_faces: ConstIndexArray) {
        ...
        let potential_max = if non_linear != 0 { deeper } else { self.has_irreg_faces as i32 };

        let mut more = FeatureMask::default();
        more.initialize(&options, self.reg_face_size);
        let mut less = more;
        if shallow < potential_max { less.reduce(&options); }
            ...
            let any_selected = {
                ...
                if i > 1 {
                    Self::select_features(
                        &mut selector, level_mask, self.reg_face_size,
                        self.has_holes, ConstIndexArray::new(&[]), parent_level);
                } else if non_linear != 0 {
                    Self::select_features(
                        &mut selector, level_mask, self.reg_face_size,
                        self.has_holes, sf, parent_level);
                } else {
                    Self::select_linear_irreg(
                        &mut selector, self.reg_face_size,
                        self.has_holes, sf, parent_level);
                }
    ...
    pub fn get_level_internal(&self, l: i32)          -> &Level      { &self.levels[l as usize] }
    pub fn get_level_internal_mut(&mut self, l: i32)  -> &mut Level  { &mut self.levels[l as usize] }
    pub fn get_refinement_internal(&self, l: i32)     -> &Refinement { &self.refinements[l as usize] }

    pub fn has_irreg_faces_flag(&self) -> bool          { self.has_irreg_faces }
    pub fn get_reg_face_size(&self) -> i32              { self.reg_face_size }
    pub fn set_has_irreg_faces(&mut self, v: bool)      { self.has_irreg_faces = v; }
    pub fn set_has_holes_flag(&mut self, v: bool)       { self.has_holes = v; }
    ...
    fn vtag_inf_sharp_features(comp: VTag, mask: &FeatureMask) -> bool {
        ...
        if comp.inf_irregular() {
            ...
            if rule_bits & (Rule::Corner as u16) != 0 {
                return if comp.corner() { false } else { mask.select_inf_sharp_regular_corner };
            } else {
                return false;
            }
        } else {
    ...
    fn face_features(level: &Level, face: Index, mask: &FeatureMask, reg_face_size: i32) -> bool {
        ...
        if fverts.size() != reg_face_size { return true; }

        let mut vtags = [VTag::default(); 4];
        level.get_face_vtags(face, &mut vtags, -1);
        let comp = VTag::bitwise_or(&vtags[..fverts.size() as usize]);

        if comp.incid_irreg_face() { return true; }
        if comp.incomplete()       { return false; }
        if comp.non_manifold() && mask.select_non_manifold { return true; }

        if comp.xordinary() && mask.select_xordinary_interior {
            if comp.rule() == (Rule::Smooth as u16) { return true; }
        ...
        if comp.rule() == (Rule::Smooth as u16) { return false; }
    ...
    fn face_fvar_features(level: &Level, face: Index, mask: &FeatureMask, channel: i32) -> bool {
        ...
        if comp.incomplete()   { return false; }
        if comp.non_manifold() && mask.select_non_manifold { return true; }
        if comp.xordinary() && mask.select_xordinary_interior { return true; }
    ...
    fn select_features(
        ...
        let total  = if faces.is_empty() { level.get_num_faces() } else { faces.size() };
        let n_fvar = if mask.select_fvar_features { level.get_num_fvar_channels() } else { 0 };
        for fi in 0..total {
            let f: Index = if faces.is_empty() { fi } else { faces[fi] };
            if has_holes && level.is_face_hole(f) { continue; }
