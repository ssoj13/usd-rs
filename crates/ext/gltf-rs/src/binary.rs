...
pub enum Error {
    ...
    Magic([u8; 4]),
    /// Length specified in GLB header exceeds that of slice.
    Length {
...
impl<'a> Glb<'a> {
    ...
    pub fn from_slice(mut data: &'a [u8]) -> Result<Self, crate::Error> {
        let header = Header::from_reader(&mut data)
            .and_then(|header| {
                // Guard against underflow: header.length must cover at least the header itself
    ...
    pub fn from_reader<R: io::Read>(mut reader: R) -> Result<Self, crate::Error> {
        ...
        match header.version {
            2 => {
                // Guard against underflow: header.length must be >= header size
                let header_size = Header::size_of() as u32;
                ...
                let glb_len = header.length - header_size;
