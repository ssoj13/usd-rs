    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if let Some(index) = next_sparse_index {
            if *index == self.counter {
                ...
                // If values is exhausted before indices (malformed glTF), stop iteration
                next_value = self.values.next()?;
            }
        }
...
impl Item for i16 {
    fn from_slice(slice: &[u8]) -> Self {
        i16::from_le_bytes(slice[..2].try_into().unwrap())
    }
    fn zero() -> Self {
        0
    }
...
impl Item for f32 {
    fn from_slice(slice: &[u8]) -> Self {
        f32::from_le_bytes(slice[..4].try_into().unwrap())
    }
    fn zero() -> Self {
        0.0
    }
...
impl<'a, 's, T: Item> Iter<'s, T> {
    ...
    {
        // Empty accessor produces an empty iterator — avoids count()-1 underflow
        if accessor.count() == 0 {
            return Some(Iter::Sparse(SparseIter::empty(0)));
        }
        ...
        match accessor.sparse() {
            Some(sparse) => {
                ...
                // Sparse with zero overrides = base accessor with no patches
                if sparse_count == 0 {
                    return Some(Iter::Sparse(SparseIter::empty(base_count)));
                }
