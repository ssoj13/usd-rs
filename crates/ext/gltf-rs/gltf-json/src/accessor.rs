...
use crate::{Extras, Index, buffer, extensions};
...
/// Validate that a non-sparse accessor must have a bufferView.
///
/// glTF 2.0 spec: "When accessor.sparse is undefined, accessor.bufferView is required."
fn accessor_validate_hook<P, R>(accessor: &Accessor, _root: &crate::Root, path: P, report: &mut R)
where
    P: Fn() -> crate::Path,
    R: FnMut(&dyn Fn() -> crate::Path, crate::validation::Error),
{
    if accessor.buffer_view.is_none() && accessor.sparse.is_none() {
        report(
            ...
            crate::validation::Error::Missing,
        );
    }
}
