...
//! Reference: `_ref/draco/src/draco/compression/attributes/sequential_quantization_attribute_decoder.h|cc`.
//!
//! Decodes quantized integer values and dequantizes them back to float32.
...
impl SequentialQuantizationAttributeDecoder {
    ...
    fn decode_quantized_data_info(&mut self, in_buffer: &mut DecoderBuffer) -> bool {
        let attribute = if let Some(portable) = self.base.portable_attribute() {
            portable
        } else {
            self.base.attribute()
        };
        self.quantization_transform
            .decode_parameters(attribute, in_buffer)
    }
    ...
    fn decode_integer_values(
        ...
        if let Some(decoder) = self.base.decoder() {
            if decoder.bitstream_version() < bitstream_version(2, 0) {
                if !self.decode_quantized_data_info(in_buffer) {
                    return false;
                }
            }
        }
        let num_components = self.base.attribute().num_components() as i32;
        if num_components <= 0 {
            return false;
        }
    ...
    fn dequantize_values(&mut self) -> bool {
...
impl SequentialAttributeDecoderInterface for SequentialQuantizationAttributeDecoder {
    ...
    fn decode_data_needed_by_portable_transform(
        ...
        if let Some(portable) = self.base.portable_attribute_mut() {
            self.quantization_transform.transfer_to_attribute(portable)
        } else {
            false
        }
    }
