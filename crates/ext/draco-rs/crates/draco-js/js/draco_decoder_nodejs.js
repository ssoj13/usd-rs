// Node.js loader for Draco WASM decoder/encoder module.
// Mirrors the draco3d `createDecoderModule` contract.
...
module.exports = async function createDecoderModule(options = {}) {
  const wasm = await import('./draco_js.js');
  const wasmPath = resolveWasmPath(options, 'draco_js_bg.wasm');
  const wasmBinary = options.wasmBinary || fs.readFileSync(wasmPath);
  await wasm.default(wasmBinary);
  return buildModule(wasm, options);
};

function resolveWasmPath(options, fileName) {
  if (typeof options.locateFile === 'function') {
    return options.locateFile(fileName, __dirname + path.sep);
  }
