...
fn py_roundf(x: f64) -> f64 {
    (x as f32).round() as f64
}

#[pyfunction(name = "Pow")]
fn py_pow(x: f64, p: f64) -> f64 {
    usd_gf::math::pow(x, p)
}

#[pyfunction(name = "Powf")]
fn py_powf(x: f64, p: f64) -> f64 {
    (x as f32).powf(p as f32) as f64
}

#[pyfunction(name = "Clamp")]
fn py_clamp(value: f64, min: f64, max: f64) -> f64 {
    usd_gf::math::clamp(value, min, max)
}
...
fn py_mod(a: f64, b: f64) -> f64 {
    usd_gf::math::modulo(a, b)
}

#[pyfunction(name = "Modf")]
fn py_modf(a: f64, b: f64) -> f64 {
    usd_gf::math::modulo_f32(a as f32, b as f32) as f64
}
...
fn py_rad_to_deg(r: f64) -> f64 {
    usd_gf::math::radians_to_degrees(r)
}

#[pyfunction(name = "DegreesToRadians")]
fn py_deg_to_rad(d: f64) -> f64 {
    usd_gf::math::degrees_to_radians(d)
}
    ...
    if let (Ok(at), Ok(bt)) = (a_tuple, b_tuple) {
        if at.len() != bt.len() {
            return Err(PyTypeError::new_err(
                "Dot: vectors must have same dimension",
            ));
        }
...
fn py_lerp(
    ...
    // Scalar lerp
    if let (Ok(va), Ok(vb)) = (a.extract::<f64>(), b.extract::<f64>()) {
        return Ok(usd_gf::math::lerp(alpha, va, vb)
            .into_pyobject(py)?
            .into_any()
            .unbind());
    }
    // Vec3d lerp
    if let (Ok(va), Ok(vb)) = (
        a.extract::<PyRef<'_, vec::PyVec3d>>(),
        b.extract::<PyRef<'_, vec::PyVec3d>>(),
    ) {
    ...
    // Vec2d lerp
    if let (Ok(va), Ok(vb)) = (
        a.extract::<PyRef<'_, vec::PyVec2d>>(),
        b.extract::<PyRef<'_, vec::PyVec2d>>(),
    ) {
    ...
    // Vec4d lerp
    if let (Ok(va), Ok(vb)) = (
        a.extract::<PyRef<'_, vec::PyVec4d>>(),
        b.extract::<PyRef<'_, vec::PyVec4d>>(),
    ) {
...
fn py_is_close(
    ...
    if let (Ok(va), Ok(vb)) = (
        ...
        return Ok((va.0.x - vb.0.x).abs() <= tolerance
    ...
    if let (Ok(va), Ok(vb)) = (
        ...
        return Ok((va.0.x - vb.0.x).abs() <= tolerance && (va.0.y - vb.0.y).abs() <= tolerance);
    }
    // Vec4d
    if let (Ok(va), Ok(vb)) = (
        a.extract::<PyRef<'_, vec::PyVec4d>>(),
    ...
    // Vec3f
    if let (Ok(va), Ok(vb)) = (
        a.extract::<PyRef<'_, vec::PyVec3f>>(),
        b.extract::<PyRef<'_, vec::PyVec3f>>(),
    ) {
        let tol = tolerance as f32;
        return Ok((va.0.x - vb.0.x).abs() <= tol
            && (va.0.y - vb.0.y).abs() <= tol
            && (va.0.z - vb.0.z).abs() <= tol);
    }
    // Color — component-wise comparison of RGB
    if let (Ok(ca), Ok(cb)) = (
        a.extract::<PyRef<'_, geo::PyColor>>(),
        b.extract::<PyRef<'_, geo::PyColor>>(),
    ) {
        let ra = ca.0.rgb();
        let rb = cb.0.rgb();
        let tol = tolerance as f32;
        return Ok((ra.x - rb.x).abs() <= tol
            && (ra.y - rb.y).abs() <= tol
            && (ra.z - rb.z).abs() <= tol);
    }
    ...
    if let (Ok(la), Ok(lb)) = (a.extract::<Vec<f64>>(), b.extract::<Vec<f64>>()) {
        if la.len() != lb.len() {
            return Err(PyTypeError::new_err(
                "IsClose: sequences must have same length",
            ));
        }
        return Ok(la
            .iter()
...
fn py_find_closest_points(
    ...
    // Ray + Line
    if let (Ok(ray), Ok(line)) = (
        a.extract::<PyRef<'_, geo::PyRay>>(),
        b.extract::<PyRef<'_, geo::PyLine>>(),
    ) {
        match usd_gf::find_closest_points_ray_line(&ray.0, &line.0) {
            ...
            None => {
                let result = (
                    false,
                    vec::PyVec3d(usd_gf::Vec3d::default()),
                    vec::PyVec3d(usd_gf::Vec3d::default()),
                    0.0_f64,
                    0.0_f64,
                );
    ...
    // Ray + LineSeg
    if let (Ok(ray), Ok(seg)) = (
        a.extract::<PyRef<'_, geo::PyRay>>(),
        b.extract::<PyRef<'_, geo::PyLineSeg>>(),
    ) {
    ...
    // LineSeg + LineSeg
    if let (Ok(s1), Ok(s2)) = (
        a.extract::<PyRef<'_, geo::PyLineSeg>>(),
        b.extract::<PyRef<'_, geo::PyLineSeg>>(),
    ) {
    ...
    Err(PyTypeError::new_err(
        "FindClosestPoints: unsupported argument types",
    ))
}
...
fn py_get_homogenized(py: Python<'_>, v: &Bound<'_, pyo3::PyAny>) -> PyResult<Py<pyo3::PyAny>> {
    if let Ok(vd) = v.extract::<PyRef<'_, vec::PyVec4d>>() {
        return Ok(vec::PyVec4d(usd_gf::homogenize(vd.0))
            .into_pyobject(py)?
            .into_any()
            .unbind());
    }
    if let Ok(vf) = v.extract::<PyRef<'_, vec::PyVec4f>>() {
        return Ok(vec::PyVec4f(usd_gf::homogenize_f(vf.0))
            .into_pyobject(py)?
            .into_any()
            .unbind());
    }
    Err(PyTypeError::new_err(
        "GetHomogenized: expected Vec4d or Vec4f",
    ))
}
...
fn py_homogeneous_cross(
    ...
    if let (Ok(ad), Ok(bd)) = (
        ...
        return Ok(vec::PyVec4d(usd_gf::homogeneous_cross(ad.0, bd.0))
            .into_pyobject(py)?
            .into_any()
            .unbind());
    }
    if let (Ok(af), Ok(bf)) = (
        a.extract::<PyRef<'_, vec::PyVec4f>>(),
        b.extract::<PyRef<'_, vec::PyVec4f>>(),
    ) {
        return Ok(vec::PyVec4f(usd_gf::homogeneous_cross_f(af.0, bf.0))
            .into_pyobject(py)?
            .into_any()
            .unbind());
    }
    Err(PyTypeError::new_err(
        "HomogeneousCross: expected Vec4d or Vec4f",
    ))
}
...
fn py_project(py: Python<'_>, v: &Bound<'_, pyo3::PyAny>) -> PyResult<Py<pyo3::PyAny>> {
    if let Ok(vd) = v.extract::<PyRef<'_, vec::PyVec4d>>() {
        return Ok(vec::PyVec3d(usd_gf::project(vd.0))
            .into_pyobject(py)?
            .into_any()
            .unbind());
    }
    if let Ok(vf) = v.extract::<PyRef<'_, vec::PyVec4f>>() {
        return Ok(vec::PyVec3f(usd_gf::project_f(vf.0))
            .into_pyobject(py)?
            .into_any()
            .unbind());
    }
...
fn py_get_length(v: &Bound<'_, pyo3::PyAny>) -> PyResult<f64> {
    if let Ok(vd) = v.extract::<PyRef<'_, vec::PyVec3d>>() {
        return Ok(vd.0.length());
    }
    if let Ok(vf) = v.extract::<PyRef<'_, vec::PyVec3f>>() {
        return Ok(vf.0.length() as f64);
    }
    if let Ok(vd) = v.extract::<PyRef<'_, vec::PyVec2d>>() {
        return Ok(vd.0.length());
    }
    if let Ok(vf) = v.extract::<PyRef<'_, vec::PyVec2f>>() {
        return Ok(vf.0.length() as f64);
    }
    if let Ok(vd) = v.extract::<PyRef<'_, vec::PyVec4d>>() {
        return Ok(vd.0.length());
...
fn py_apply_gamma(
    ...
    if let Ok(vd) = v.extract::<PyRef<'_, vec::PyVec3d>>() {
        return Ok(vec::PyVec3d(usd_gf::apply_gamma(vd.0, gamma))
            .into_pyobject(py)?
            .into_any()
            .unbind());
    }
    if let Ok(vf) = v.extract::<PyRef<'_, vec::PyVec3f>>() {
        return Ok(vec::PyVec3f(usd_gf::apply_gamma(vf.0, gamma))
            .into_pyobject(py)?
            .into_any()
            .unbind());
    }
    if let Ok(vd) = v.extract::<PyRef<'_, vec::PyVec4d>>() {
        return Ok(vec::PyVec4d(usd_gf::apply_gamma(vd.0, gamma))
            .into_pyobject(py)?
            .into_any()
            .unbind());
    }
    if let Ok(vf) = v.extract::<PyRef<'_, vec::PyVec4f>>() {
        return Ok(vec::PyVec4f(usd_gf::apply_gamma(vf.0, gamma))
            .into_pyobject(py)?
            .into_any()
            .unbind());
    }
    if let Ok(vh) = v.extract::<PyRef<'_, vec::PyVec3h>>() {
        return Ok(vec::PyVec3h(usd_gf::apply_gamma(vh.0, gamma))
            .into_pyobject(py)?
            .into_any()
            .unbind());
    }
    if let Ok(vh) = v.extract::<PyRef<'_, vec::PyVec4h>>() {
        return Ok(vec::PyVec4h(usd_gf::apply_gamma(vh.0, gamma))
            .into_pyobject(py)?
            .into_any()
            .unbind());
    }
...
fn py_get_display_gamma() -> f64 {
    usd_gf::get_display_gamma()
}
...
fn py_linear_to_display(py: Python<'_>, v: &Bound<'_, pyo3::PyAny>) -> PyResult<Py<pyo3::PyAny>> {
    if let Ok(vd) = v.extract::<PyRef<'_, vec::PyVec3d>>() {
        return Ok(vec::PyVec3d(usd_gf::linear_to_display(vd.0))
            .into_pyobject(py)?
            .into_any()
            .unbind());
    }
    if let Ok(vf) = v.extract::<PyRef<'_, vec::PyVec3f>>() {
        return Ok(vec::PyVec3f(usd_gf::linear_to_display(vf.0))
            .into_pyobject(py)?
            .into_any()
            .unbind());
    }
    if let Ok(vd) = v.extract::<PyRef<'_, vec::PyVec4d>>() {
        return Ok(vec::PyVec4d(usd_gf::linear_to_display(vd.0))
            .into_pyobject(py)?
            .into_any()
            .unbind());
    }
    if let Ok(vf) = v.extract::<PyRef<'_, vec::PyVec4f>>() {
        return Ok(vec::PyVec4f(usd_gf::linear_to_display(vf.0))
            .into_pyobject(py)?
            .into_any()
            .unbind());
    }
    if let Ok(vh) = v.extract::<PyRef<'_, vec::PyVec3h>>() {
        return Ok(vec::PyVec3h(usd_gf::linear_to_display(vh.0))
            .into_pyobject(py)?
            .into_any()
            .unbind());
    }
    if let Ok(vh) = v.extract::<PyRef<'_, vec::PyVec4h>>() {
        return Ok(vec::PyVec4h(usd_gf::linear_to_display(vh.0))
            .into_pyobject(py)?
            .into_any()
            .unbind());
    }
    Err(PyTypeError::new_err(
        "ConvertLinearToDisplay: unsupported type",
    ))
}
...
fn py_display_to_linear(py: Python<'_>, v: &Bound<'_, pyo3::PyAny>) -> PyResult<Py<pyo3::PyAny>> {
    if let Ok(vd) = v.extract::<PyRef<'_, vec::PyVec3d>>() {
        return Ok(vec::PyVec3d(usd_gf::display_to_linear(vd.0))
            .into_pyobject(py)?
            .into_any()
            .unbind());
    }
    if let Ok(vf) = v.extract::<PyRef<'_, vec::PyVec3f>>() {
        return Ok(vec::PyVec3f(usd_gf::display_to_linear(vf.0))
            .into_pyobject(py)?
            .into_any()
            .unbind());
    }
    if let Ok(vd) = v.extract::<PyRef<'_, vec::PyVec4d>>() {
        return Ok(vec::PyVec4d(usd_gf::display_to_linear(vd.0))
            .into_pyobject(py)?
            .into_any()
            .unbind());
    }
    if let Ok(vf) = v.extract::<PyRef<'_, vec::PyVec4f>>() {
        return Ok(vec::PyVec4f(usd_gf::display_to_linear(vf.0))
            .into_pyobject(py)?
            .into_any()
            .unbind());
    }
    if let Ok(vh) = v.extract::<PyRef<'_, vec::PyVec3h>>() {
        return Ok(vec::PyVec3h(usd_gf::display_to_linear(vh.0))
            .into_pyobject(py)?
            .into_any()
            .unbind());
    }
    if let Ok(vh) = v.extract::<PyRef<'_, vec::PyVec4h>>() {
        return Ok(vec::PyVec4h(usd_gf::display_to_linear(vh.0))
            .into_pyobject(py)?
            .into_any()
            .unbind());
    }
    Err(PyTypeError::new_err(
        "ConvertDisplayToLinear: unsupported type",
    ))
}
