...
fn idx2(i: isize) -> PyResult<usize> {
    ...
    if j < 0 || j >= 2 {
        Err(PyIndexError::new_err("matrix row index out of range"))
    } else {
        Ok(j as usize)
    }
}
...
impl PyMatrix2d {
    #[classattr]
    #[pyo3(name = "dimension")]
    const DIMENSION: (usize, usize) = (2, 2);
    ...
    fn new(args: &Bound<'_, pyo3::types::PyTuple>) -> PyResult<Self> {
        let n = args.len();
        if n == 0 {
            return Ok(Self(Matrix2d::from_diagonal(1.0, 1.0)));
        }
        if n == 1 {
            let obj = args.get_item(0)?;
            if let Ok(v) = obj.extract::<f64>() {
                return Ok(Self(Matrix2d::from_diagonal(v, v)));
            }
            if let Ok(m) = obj.extract::<PyRef<'_, PyMatrix2d>>() {
                return Ok(Self(m.0));
            }
        ...
        Err(pyo3::exceptions::PyTypeError::new_err(
            "Matrix2d: expected (), (scalar), (Vec2d), (Matrix2d), or (4 floats)",
        ))
    }
    ...
    fn __int__(&self) -> PyResult<i64> {
        Err(PyValueError::new_err("cannot convert Matrix to int"))
    }
    ...
    fn __getitem__(
        ...
        Ok(
            super::vec::PyVec2d(usd_gf::Vec2d::new(self.0[r][0], self.0[r][1]))
                .into_pyobject(py)?
                .into_any()
                .unbind(),
        )
    ...
    fn __setitem__(
        ...
        if row.len() != 2 {
            return Err(PyIndexError::new_err("expected 2 elements"));
        }
        self.0[r][0] = row[0];
        self.0[r][1] = row[1];
        Ok(())
    ...
    fn set(&mut self, args: &Bound<'_, pyo3::types::PyTuple>) -> PyResult<Self> {
        if args.len() != 4 {
            return Err(PyValueError::new_err("Set: expected 4 floats"));
        }
        self.0[0][0] = args.get_item(0)?.extract()?;
        self.0[0][1] = args.get_item(1)?.extract()?;
        self.0[1][0] = args.get_item(2)?.extract()?;
        self.0[1][1] = args.get_item(3)?.extract()?;
        Ok(self.clone())
    }
    #[pyo3(name = "SetZero")]
    fn set_zero(&mut self) {
        self.0 = Matrix2d::zero();
    }
    #[pyo3(name = "SetIdentity")]
    fn set_identity(&mut self) {
        self.0 = Matrix2d::identity();
    }
    #[pyo3(name = "SetDiagonal")]
    fn set_diagonal(&mut self, py: Python<'_>, s: &Bound<'_, pyo3::PyAny>) -> PyResult<Self> {
        if let Ok(v) = s.extract::<f64>() {
            self.0.set_diagonal(v, v);
            return Ok(self.clone());
        }
        if let Ok(v) = s.extract::<PyRef<'_, super::vec::PyVec2d>>() {
            self.0.set_diagonal(v.0.x, v.0.y);
            return Ok(self.clone());
        }
        let _ = py;
        Err(pyo3::exceptions::PyTypeError::new_err(
            "SetDiagonal: expected scalar or Vec2d",
        ))
    }
    ...
    #[pyo3(name = "GetRow")]
    fn get_row(&self, i: usize) -> PyResult<super::vec::PyVec2d> {
        if i >= 2 {
            return Err(PyIndexError::new_err("row index out of range"));
        }
        Ok(super::vec::PyVec2d(usd_gf::Vec2d::new(
            self.0[i][0],
            self.0[i][1],
        )))
    }
    #[pyo3(name = "GetColumn")]
    fn get_column(&self, j: usize) -> PyResult<super::vec::PyVec2d> {
        if j >= 2 {
            return Err(PyIndexError::new_err("col index out of range"));
        }
        Ok(super::vec::PyVec2d(usd_gf::Vec2d::new(
            self.0[0][j],
            self.0[1][j],
    ...
    #[pyo3(name = "SetRow")]
    fn set_row(&mut self, i: usize, v: &super::vec::PyVec2d) -> PyResult<()> {
        if i >= 2 {
            return Err(PyIndexError::new_err("row index out of range"));
        }
        self.0[i][0] = v.0.x;
    ...
    #[pyo3(name = "SetColumn")]
    fn set_column(&mut self, j: usize, v: &super::vec::PyVec2d) -> PyResult<()> {
        if j >= 2 {
            return Err(PyIndexError::new_err("col index out of range"));
        }
        self.0[0][j] = v.0.x;
...
impl PyMatrix2f {
    ...
    fn new(args: &Bound<'_, pyo3::types::PyTuple>) -> PyResult<Self> {
        let n = args.len();
        if n == 0 {
            return Ok(Self(Matrix2f::from_diagonal(1.0, 1.0)));
        }
        if n == 1 {
            let obj = args.get_item(0)?;
            if let Ok(v) = obj.extract::<f32>() {
                return Ok(Self(Matrix2f::from_diagonal(v, v)));
            }
            if let Ok(m) = obj.extract::<PyRef<'_, PyMatrix2f>>() {
                return Ok(Self(m.0));
            }
        ...
        Err(pyo3::exceptions::PyTypeError::new_err(
            "Matrix2f: unsupported constructor",
        ))
    }
    ...
    fn __getitem__(
        ...
        let r = idx2(key.extract()?)?;
        Ok(
            super::vec::PyVec2f(usd_gf::Vec2f::new(self.0[r][0], self.0[r][1]))
                .into_pyobject(py)?
                .into_any()
                .unbind(),
        )
    ...
    #[pyo3(name = "SetZero")]
    fn set_zero(&mut self) {
        self.0 = Matrix2f::zero();
    }
    #[pyo3(name = "SetIdentity")]
    fn set_identity(&mut self) {
        self.0 = Matrix2f::identity();
    }
    #[pyo3(name = "SetDiagonal")]
    fn set_diagonal(&mut self, py: Python<'_>, s: &Bound<'_, pyo3::PyAny>) -> PyResult<Self> {
        ...
        let _ = py;
        Err(pyo3::exceptions::PyTypeError::new_err(
            "SetDiagonal: expected scalar or Vec2f",
        ))
    }
    #[pyo3(name = "GetDeterminant")]
    fn get_determinant(&self) -> f32 {
        self.0.determinant()
    }
    #[pyo3(name = "GetInverse")]
    fn get_inverse(&self) -> Self {
    ...
    #[pyo3(name = "GetRow")]
    fn get_row(&self, i: usize) -> PyResult<super::vec::PyVec2f> {
        if i >= 2 {
            return Err(PyIndexError::new_err("row index out of range"));
        }
        Ok(super::vec::PyVec2f(usd_gf::Vec2f::new(
            self.0[i][0],
            self.0[i][1],
        )))
    }
    #[pyo3(name = "GetColumn")]
    fn get_column(&self, j: usize) -> PyResult<super::vec::PyVec2f> {
        if j >= 2 {
            return Err(PyIndexError::new_err("col index out of range"));
        }
        Ok(super::vec::PyVec2f(usd_gf::Vec2f::new(
    ...
    #[pyo3(name = "SetRow")]
    fn set_row(&mut self, i: usize, v: &super::vec::PyVec2f) -> PyResult<()> {
        if i >= 2 {
            return Err(PyIndexError::new_err("row index out of range"));
        }
        self.0[i][0] = v.0.x;
    ...
    #[pyo3(name = "SetColumn")]
    fn set_column(&mut self, j: usize, v: &super::vec::PyVec2f) -> PyResult<()> {
        if j >= 2 {
            return Err(PyIndexError::new_err("col index out of range"));
        }
        self.0[0][j] = v.0.x;
...
impl PyMatrix3d {
    #[classattr]
    #[pyo3(name = "dimension")]
    const DIMENSION: (usize, usize) = (3, 3);
    ...
    fn new(args: &Bound<'_, pyo3::types::PyTuple>) -> PyResult<Self> {
        let n = args.len();
        if n == 0 {
            return Ok(Self(Matrix3d::from_diagonal_values(1.0, 1.0, 1.0)));
        }
        if n == 1 {
            let obj = args.get_item(0)?;
            if let Ok(v) = obj.extract::<f64>() {
            ...
            if let Ok(m) = obj.extract::<PyRef<'_, PyMatrix3d>>() {
                return Ok(Self(m.0));
            }
        ...
        Err(pyo3::exceptions::PyTypeError::new_err(
            "Matrix3d: unsupported constructor",
        ))
    }
    ...
    fn __getitem__(
        ...
        Ok(
            super::vec::PyVec3d(usd_gf::Vec3d::new(self.0[r][0], self.0[r][1], self.0[r][2]))
                .into_pyobject(py)?
                .into_any()
                .unbind(),
        )
    ...
    fn __setitem__(
        ...
        let row: Vec<f64> = val.extract()?;
        if row.len() != 3 {
            return Err(PyIndexError::new_err("expected 3 elements"));
        }
        self.0[r][0] = row[0];
        self.0[r][1] = row[1];
        self.0[r][2] = row[2];
    ...
    fn set(&mut self, args: &Bound<'_, pyo3::types::PyTuple>) -> PyResult<Self> {
        if args.len() != 9 {
            return Err(PyValueError::new_err("Set: expected 9 floats"));
        }
        for i in 0..3 {
            for j in 0..3 {
                self.0[i][j] = args.get_item(i * 3 + j)?.extract()?;
            }
        }
        Ok(self.clone())
    }
    #[pyo3(name = "SetZero")]
    fn set_zero(&mut self) {
        self.0 = Matrix3d::zero();
    }
    #[pyo3(name = "SetIdentity")]
    fn set_identity(&mut self) {
        self.0 = Matrix3d::identity();
    }
    #[pyo3(name = "SetDiagonal")]
    fn set_diagonal(&mut self, py: Python<'_>, s: &Bound<'_, pyo3::PyAny>) -> PyResult<Self> {
        if let Ok(v) = s.extract::<f64>() {
            self.0.set_diagonal(v, v, v);
            return Ok(self.clone());
        }
        if let Ok(v) = s.extract::<PyRef<'_, super::vec::PyVec3d>>() {
            self.0.set_diagonal(v.0.x, v.0.y, v.0.z);
            return Ok(self.clone());
        }
        let _ = py;
        Err(pyo3::exceptions::PyTypeError::new_err(
            "SetDiagonal: expected scalar or Vec3d",
        ))
    }
    #[pyo3(name = "Orthonormalize")]
    fn orthonormalize(&mut self) -> bool {
        self.0.orthonormalize()
    }
    #[pyo3(name = "GetOrthonormalized")]
    fn get_orthonormalized(&self) -> Self {
        Self(self.0.orthonormalized())
    }
    #[pyo3(name = "GetDeterminant")]
    fn get_determinant(&self) -> f64 {
        self.0.determinant()
    ...
    #[pyo3(name = "GetRow")]
    fn get_row(&self, i: usize) -> PyResult<Vec<f64>> {
        if i >= 3 {
            return Err(PyIndexError::new_err("row index out of range"));
        }
    ...
    #[pyo3(name = "GetColumn")]
    fn get_column(&self, j: usize) -> PyResult<Vec<f64>> {
        if j >= 3 {
            return Err(PyIndexError::new_err("col index out of range"));
        }
    ...
    #[pyo3(name = "SetRow")]
    fn set_row(&mut self, i: usize, row: Vec<f64>) -> PyResult<()> {
        if i >= 3 {
            return Err(PyIndexError::new_err("row index out of range"));
        }
        if row.len() != 3 {
    ...
    #[pyo3(name = "SetColumn")]
    fn set_column(&mut self, j: usize, col: Vec<f64>) -> PyResult<()> {
        if j >= 3 {
            return Err(PyIndexError::new_err("col index out of range"));
        }
        if col.len() != 3 {
    ...
    fn set_scale(&mut self, py: Python<'_>, s: &Bound<'_, pyo3::PyAny>) -> PyResult<Self> {
        ...
        let _ = py;
        Err(pyo3::exceptions::PyTypeError::new_err(
            "SetScale: expected scalar or Vec3d",
        ))
    }
    ...
    fn set_rotate(&mut self, py: Python<'_>, rot: &Bound<'_, pyo3::PyAny>) -> PyResult<Self> {
        ...
        let _ = py;
        Err(pyo3::exceptions::PyTypeError::new_err(
            "SetRotate: expected Rotation or Quatd",
        ))
    }
...
impl PyMatrix3f {
    ...
    fn new(args: &Bound<'_, pyo3::types::PyTuple>) -> PyResult<Self> {
        let n = args.len();
        if n == 0 {
            return Ok(Self(Matrix3f::from_diagonal_values(1.0, 1.0, 1.0)));
        }
        if n == 1 {
            let obj = args.get_item(0)?;
            if let Ok(v) = obj.extract::<f32>() {
                return Ok(Self(Matrix3f::from_diagonal_values(v, v, v)));
            }
            if let Ok(m) = obj.extract::<PyRef<'_, PyMatrix3f>>() {
                return Ok(Self(m.0));
            }
            ...
            if let Ok(r) = obj.extract::<PyRef<'_, super::geo::PyRotation>>() {
                ...
                for i in 0..3 {
                    for j in 0..3 {
                        m3f[i][j] = m3d[i][j] as f32;
                    }
                }
                return Ok(Self(m3f));
            }
            if let Ok(q) = obj.extract::<PyRef<'_, super::quat::PyQuatf>>() {
                let qd = usd_gf::Quatd::new(
                    q.0.real() as f64,
                    usd_gf::Vec3d::new(
                        q.0.imaginary().x as f64,
                        q.0.imaginary().y as f64,
                        q.0.imaginary().z as f64,
                    ),
                );
        ...
        Err(pyo3::exceptions::PyTypeError::new_err(
            "Matrix3f: unsupported constructor",
        ))
    }
    ...
    fn __getitem__(
        ...
        let r = idx3(key.extract()?)?;
        Ok(
            super::vec::PyVec3f(usd_gf::Vec3f::new(self.0[r][0], self.0[r][1], self.0[r][2]))
                .into_pyobject(py)?
                .into_any()
                .unbind(),
        )
    ...
    #[pyo3(name = "SetZero")]
    fn set_zero(&mut self) {
        self.0 = Matrix3f::zero();
    }
    #[pyo3(name = "SetIdentity")]
    fn set_identity(&mut self) {
        self.0 = Matrix3f::identity();
    }
    #[pyo3(name = "SetDiagonal")]
    fn set_diagonal(&mut self, py: Python<'_>, s: &Bound<'_, pyo3::PyAny>) -> PyResult<Self> {
        if let Ok(v) = s.extract::<f32>() {
        ...
        let _ = py;
        Err(pyo3::exceptions::PyTypeError::new_err(
            "SetDiagonal: expected scalar or Vec3f",
        ))
    }
    #[pyo3(name = "GetDeterminant")]
    fn get_determinant(&self) -> f32 {
    ...
    #[pyo3(name = "GetRow")]
    fn get_row(&self, i: usize) -> PyResult<super::vec::PyVec3f> {
        if i >= 3 {
            return Err(PyIndexError::new_err("row index out of range"));
        }
        Ok(super::vec::PyVec3f(usd_gf::Vec3f::new(
            self.0[i][0],
            self.0[i][1],
            self.0[i][2],
        )))
    ...
    #[pyo3(name = "GetColumn")]
    fn get_column(&self, j: usize) -> PyResult<super::vec::PyVec3f> {
        if j >= 3 {
            return Err(PyIndexError::new_err("col index out of range"));
        }
        Ok(super::vec::PyVec3f(usd_gf::Vec3f::new(
    ...
    #[pyo3(name = "SetRow")]
    fn set_row(&mut self, i: usize, v: &super::vec::PyVec3f) -> PyResult<()> {
        if i >= 3 {
            return Err(PyIndexError::new_err("row index out of range"));
        }
        self.0[i][0] = v.0.x;
    ...
    #[pyo3(name = "SetColumn")]
    fn set_column(&mut self, j: usize, v: &super::vec::PyVec3f) -> PyResult<()> {
        if j >= 3 {
            return Err(PyIndexError::new_err("col index out of range"));
        }
        self.0[0][j] = v.0.x;
    ...
    fn set_scale(&mut self, py: Python<'_>, s: &Bound<'_, pyo3::PyAny>) -> PyResult<Self> {
        ...
        let _ = py;
        Err(pyo3::exceptions::PyTypeError::new_err(
            "SetScale: expected scalar or Vec3f",
        ))
    }
    ...
    fn set_rotate(&mut self, py: Python<'_>, rot: &Bound<'_, pyo3::PyAny>) -> PyResult<Self> {
        ...
        let _ = py;
        Err(pyo3::exceptions::PyTypeError::new_err(
            "SetRotate: expected Rotation or Quatf",
        ))
    }
}
...
impl PyMatrix4d {
    #[classattr]
    #[pyo3(name = "dimension")]
    const DIMENSION: (usize, usize) = (4, 4);
    ...
    fn new(args: &Bound<'_, pyo3::types::PyTuple>) -> PyResult<Self> {
        let n = args.len();
        if n == 0 {
            return Ok(Self(Matrix4d::from_diagonal_values(1.0, 1.0, 1.0, 1.0)));
        }
        if n == 1 {
            let obj = args.get_item(0)?;
            if let Ok(v) = obj.extract::<f64>() {
            ...
            if let Ok(m) = obj.extract::<PyRef<'_, PyMatrix4d>>() {
                return Ok(Self(m.0));
            }
            if let Ok(r) = obj.extract::<PyRef<'_, super::geo::PyRotation>>() {
            ...
            if let Ok(v) = obj.extract::<PyRef<'_, super::vec::PyVec4d>>() {
                return Ok(Self(Matrix4d::from_diagonal_values(
        ...
        Err(pyo3::exceptions::PyTypeError::new_err(
            "Matrix4d: expected (), (scalar), (Rotation), (Matrix3d, Vec3d), or (16 floats)",
        ))
    }
    ...
    fn __getitem__(
        ...
        Ok(super::vec::PyVec4d(usd_gf::Vec4d::new(
            self.0[r][0],
            self.0[r][1],
            self.0[r][2],
            self.0[r][3],
    ...
    fn __setitem__(
        ...
        let row: Vec<f64> = val.extract()?;
        if row.len() != 4 {
            return Err(PyIndexError::new_err("expected 4 elements"));
        }
        self.0[r][0] = row[0];
        self.0[r][1] = row[1];
        self.0[r][2] = row[2];
    ...
    fn __mul__(&self, py: Python<'_>, o: &Bound<'_, pyo3::PyAny>) -> PyResult<Py<pyo3::PyAny>> {
        ...
        if let Ok(v) = o.extract::<PyRef<'_, super::vec::PyVec4d>>() {
            return Ok(super::vec::PyVec4d(self.0 * v.0)
                .into_pyobject(py)?
                .into_any()
                .unbind());
        }
    ...
    fn set(&mut self, args: &Bound<'_, pyo3::types::PyTuple>) -> PyResult<Self> {
        if args.len() != 16 {
            return Err(PyValueError::new_err("Set: expected 16 floats"));
        }
        for i in 0..4 {
            for j in 0..4 {
                self.0[i][j] = args.get_item(i * 4 + j)?.extract()?;
            }
        }
        Ok(self.clone())
    }
    #[pyo3(name = "SetZero")]
    fn set_zero(&mut self) {
        self.0 = Matrix4d::zero();
    }
    #[pyo3(name = "SetIdentity")]
    fn set_identity(&mut self) {
        self.0 = Matrix4d::identity();
    }
    #[pyo3(name = "SetDiagonal")]
    fn set_diagonal(&mut self, py: Python<'_>, s: &Bound<'_, pyo3::PyAny>) -> PyResult<Self> {
        if let Ok(v) = s.extract::<f64>() {
            self.0.set_diagonal(v, v, v, v);
            return Ok(self.clone());
        }
        if let Ok(v) = s.extract::<PyRef<'_, super::vec::PyVec4d>>() {
            self.0.set_diagonal(v.0.x, v.0.y, v.0.z, v.0.w);
            return Ok(self.clone());
        }
        let _ = py;
        Err(pyo3::exceptions::PyTypeError::new_err(
            "SetDiagonal: expected scalar or Vec4d",
        ))
    }
    #[pyo3(name = "Orthonormalize")]
    fn orthonormalize(&mut self) -> bool {
    ...
    fn factor(
        ...
        match self.0.factor() {
            ...
            None => (
                ...
                super::vec::PyVec3d(usd_gf::Vec3d::default()),
                Self(Matrix4d::identity()),
                super::vec::PyVec3d(usd_gf::Vec3d::default()),
                Self(Matrix4d::identity()),
            ),
    ...
    fn set_look_at(
        &mut self,
        eye: &super::vec::PyVec3d,
        center: &super::vec::PyVec3d,
        up: &super::vec::PyVec3d,
    ) -> Self {
    ...
    fn set_scale(&mut self, py: Python<'_>, s: &Bound<'_, pyo3::PyAny>) -> PyResult<Self> {
        ...
        if let Ok(tup) = s.extract::<(f64, f64, f64)>() {
            self.0
                .set_scale_vec(&usd_gf::Vec3d::new(tup.0, tup.1, tup.2));
            return Ok(self.clone());
        }
        let _ = py;
        Err(pyo3::exceptions::PyTypeError::new_err(
    ...
    fn set_rotate(&mut self, py: Python<'_>, rot: &Bound<'_, pyo3::PyAny>) -> PyResult<Self> {
        ...
        let _ = py;
        Err(pyo3::exceptions::PyTypeError::new_err(
            "SetRotate: expected Rotation, Quatd, or Matrix3d",
        ))
    }
    ...
    fn set_rotate_only(&mut self, py: Python<'_>, rot: &Bound<'_, pyo3::PyAny>) -> PyResult<Self> {
        ...
        if let Ok(r) = rot.extract::<pyo3::PyRef<'_, super::geo::PyRotation>>() {
            ...
            return Err(pyo3::exceptions::PyTypeError::new_err(
                "SetRotateOnly: expected Rotation, Quatd, or Matrix3d",
            ));
        }
        self.0[3][0] = t.x;
        self.0[3][1] = t.y;
    ...
    fn set_translate(&mut self, t: &Bound<'_, pyo3::PyAny>) -> PyResult<Self> {
        if let Ok(v) = t.extract::<PyRef<'_, super::vec::PyVec3d>>() {
            ...
            self.0.set_translate(&usd_gf::Vec3d::new(x, y, z));
        } else {
            return Err(pyo3::exceptions::PyTypeError::new_err(
    ...
    fn set_translate_only(&mut self, py: Python<'_>, t: &Bound<'_, pyo3::PyAny>) -> PyResult<Self> {
        if let Ok(v) = t.extract::<PyRef<'_, super::vec::PyVec3d>>() {
            ...
            self.0
                .set_translate_only(&usd_gf::Vec3d::new(tup.0, tup.1, tup.2));
        } else {
            return Err(pyo3::exceptions::PyTypeError::new_err(
                "SetTranslateOnly: expected Vec3d or tuple",
            ));
        }
        let _ = py;
        Ok(self.clone())
    ...
    fn transform_affine(
        ...
        let (dx, dy, dz) = if let Ok(vd) = v.extract::<pyo3::PyRef<'_, super::vec::PyVec3d>>() {
            ...
            return Err(pyo3::exceptions::PyTypeError::new_err(
                "TransformAffine: expected Vec3d or Vec3f",
            ));
        };
        let m = &self.0;
        let r = usd_gf::Vec3d::new(
    ...
    fn transform(&self, py: Python<'_>, v: &Bound<'_, pyo3::PyAny>) -> PyResult<Py<pyo3::PyAny>> {
        let (dx, dy, dz) = if let Ok(vd) = v.extract::<pyo3::PyRef<'_, super::vec::PyVec3d>>() {
            ...
            return Err(pyo3::exceptions::PyTypeError::new_err(
                "Transform: expected Vec3d or Vec3f",
            ));
        };
        let m = &self.0;
        ...
        let r = if w != 0.0 {
            usd_gf::Vec3d::new(x / w, y / w, z / w)
        } else {
            usd_gf::Vec3d::new(x, y, z)
        };
        Ok(super::vec::PyVec3d(r)
            .into_pyobject(py)?
            .into_any()
...
impl PyMatrix4f {
    ...
    fn new(args: &Bound<'_, pyo3::types::PyTuple>) -> PyResult<Self> {
        let n = args.len();
        if n == 0 {
            return Ok(Self(Matrix4f::from_diagonal_values(1.0, 1.0, 1.0, 1.0)));
        }
        if n == 1 {
            let obj = args.get_item(0)?;
            if let Ok(v) = obj.extract::<f32>() {
            ...
            if let Ok(m) = obj.extract::<PyRef<'_, PyMatrix4f>>() {
                return Ok(Self(m.0));
            }
            if let Ok(v) = obj.extract::<PyRef<'_, super::vec::PyVec4f>>() {
                return Ok(Self(Matrix4f::from_diagonal_values(
                    v.0.x, v.0.y, v.0.z, v.0.w,
                )));
            }
            if let Ok(r) = obj.extract::<PyRef<'_, super::geo::PyRotation>>() {
        ...
        Err(pyo3::exceptions::PyTypeError::new_err(
            "Matrix4f: unsupported constructor",
        ))
    }
    ...
    fn __getitem__(
        ...
        let r = idx4(key.extract()?)?;
        Ok(super::vec::PyVec4f(usd_gf::Vec4f::new(
            self.0[r][0],
            self.0[r][1],
            self.0[r][2],
            self.0[r][3],
    ...
    fn set_diagonal(&mut self, py: Python<'_>, s: &Bound<'_, pyo3::PyAny>) -> PyResult<Self> {
        ...
        let _ = py;
        Err(pyo3::exceptions::PyTypeError::new_err(
            "SetDiagonal: expected scalar or Vec4f",
        ))
    }
    /// Factor(eps?) -> (success, scaleOrientation, scale, rotation, translation, projection)
    #[pyo3(name = "Factor")]
        ...
        match m4d.factor() {
            Some((r, s, u, t, p)) => {
                ...
                (
                    true,
                    Self(to4f(r)),
                    super::vec::PyVec3f(usd_gf::Vec3f::new(s.x as f32, s.y as f32, s.z as f32)),
                    Self(to4f(u)),
                    super::vec::PyVec3f(usd_gf::Vec3f::new(t.x as f32, t.y as f32, t.z as f32)),
                    Self(to4f(p)),
                )
            }
            None => (
                ...
                super::vec::PyVec3f(usd_gf::Vec3f::default()),
                Self(Matrix4f::identity()),
                super::vec::PyVec3f(usd_gf::Vec3f::default()),
                Self(Matrix4f::identity()),
            ),
    ...
    fn transform(&self, v: &super::vec::PyVec3f) -> super::vec::PyVec3f {
        ...
        if w != 0.0 {
            super::vec::PyVec3f(usd_gf::Vec3f::new(x / w, y / w, z / w))
        } else {
            super::vec::PyVec3f(usd_gf::Vec3f::new(x, y, z))
        }
    }
}
