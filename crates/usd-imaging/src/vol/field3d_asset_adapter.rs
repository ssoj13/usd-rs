//! Adapter for Field3DAsset field primitives.
//!
//! Handles imaging translation for USD Field3DAsset prims into Hydra
//! volume field representations.
//!
//! # C++ Reference
//!
//! Port of `pxr/usdImaging/usdVolImaging/field3dAssetAdapter.h`
...
/// Adapter for Field3DAsset field primitives.
///
/// Translates UsdVolField3DAsset prims into Hydra imaging primitives,
/// providing access to Field3D volumetric data files.
pub struct Field3DAssetAdapter;

impl Field3DAssetAdapter {
    /// Creates a new Field3DAsset adapter.
    pub fn new() -> Self {
        Self
    }
}

impl Default for Field3DAssetAdapter {
    fn default() -> Self {
        Self::new()
    }
}

impl FieldAdapter for Field3DAssetAdapter {
    ...
    fn get_imaging_subprim_type(&self, _prim: &Prim, subprim: &Token) -> Option<Token> {
        if subprim.is_empty() {
            Some(USD_VOL_IMAGING_TOKENS.field3d_asset.clone())
        } else {
            None
        }
    }
    ...
    fn get(&self, prim: &Prim, _cache_path: &Path, key: &Token, time: TimeCode) -> Option<Value> {
        // Handle Field3DAsset specific attributes
        match key.as_str() {
            "filePath" => {
            ...
            "fieldPurpose" => {
                if let Some(attr) = prim.get_attribute("fieldPurpose") {
                    if let Some(value) = attr.get(time) {
                        return Some(value);
                    }
                }
...
mod tests {
    ...
    fn test_adapter_creation() {
        ...
        assert_eq!(adapter.get_prim_type_token().as_str(), "Field3DAsset");
    }
