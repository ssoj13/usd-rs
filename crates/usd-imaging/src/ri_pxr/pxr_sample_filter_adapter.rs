//! RenderMan sample filter adapter.

use std::sync::Arc;

use super::data_source_render_terminal::{DataSourceRenderTerminalPrim, sample_filter_resource_locator};
...
pub struct PxrSampleFilterAdapter;

impl PxrSampleFilterAdapter {
    pub fn new() -> Self {
        Self
    }
}

impl PrimAdapter for PxrSampleFilterAdapter {
    ...
    fn get_imaging_subprim_data(
        ...
        let ds = DataSourceRenderTerminalPrim::new(
            ...
            prim.clone(),
            Token::new("sampleFilter"),
            Token::new("ri:sampleFilter:shaderId"),
            stage_globals,
        );
        ds.get(&Token::new("sampleFilter"))
            .as_ref()
    ...
    fn invalidate_imaging_subprim(
        ...
        DataSourceRenderTerminalPrim::invalidate(
            ...
            properties,
            invalidation_type,
            &sample_filter_resource_locator(),
        )
    }
}

pub type PxrSampleFilterAdapterHandle = Arc<PxrSampleFilterAdapter>;

pub fn create_pxr_sample_filter_adapter() -> Arc<dyn PrimAdapter> {
    Arc::new(PxrSampleFilterAdapter::new())
}
...
mod tests {
    ...
    #[test]
    fn test_sample_filter_type() {
        let adapter = PxrSampleFilterAdapter::new();
    ...
    #[test]
    fn test_sample_filter_data_container_exists() {
        let adapter = PxrSampleFilterAdapter::new();
        let stage = Stage::create_in_memory(usd_core::common::InitialLoadSet::LoadAll).unwrap();
