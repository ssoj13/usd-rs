//! Physics Prismatic Joint schema.
//!
//! Predefined prismatic joint type where translation along the joint axis
//! is permitted. Used for sliders and similar linear constraints.
//!
//! # C++ Reference
//!
//! Port of `pxr/usd/usdPhysics/prismaticJoint.h` and `prismaticJoint.cpp`
//!
//! # Usage
//!
//! ```ignore
//! use usd::usd_physics::PrismaticJoint;
//!
//! let joint = PrismaticJoint::define(&stage, &path)?;
//! joint.create_axis_attr(Some("X".into()))?;
//! joint.create_lower_limit_attr(Some(-10.0))?;  // distance units
//! joint.create_upper_limit_attr(Some(10.0))?;
//! ```
...
/// - `lowerLimit`: Lower distance limit. -inf means unlimited.
/// - `upperLimit`: Upper distance limit. inf means unlimited.
///
/// # Inheritance
///
...
/// # C++ Reference
///
/// Port of `UsdPhysicsPrismaticJoint` class.
#[derive(Debug, Clone)]
...
impl PrismaticJoint {
    ...
    pub const SCHEMA_TYPE_NAME: &'static str = "PhysicsPrismaticJoint";

    // =========================================================================
    // Construction
    // =========================================================================

    /// Construct a PrismaticJoint on the given prim.
    ...
    /// Construct a PrismaticJoint from another prim.
    pub fn from_prim(prim: &Prim) -> Self {
        Self::new(prim.clone())
    }

    /// Return a PrismaticJoint holding the prim at `path` on `stage`.
...
impl std::ops::Deref for PrismaticJoint {
    type Target = Joint;
...
impl From<Prim> for PrismaticJoint {
    fn from(prim: Prim) -> Self {
        Self::new(prim)
    }
}

impl From<PrismaticJoint> for Prim {
    fn from(joint: PrismaticJoint) -> Self {
        joint.joint.into()
    }
}

impl AsRef<Prim> for PrismaticJoint {
    fn as_ref(&self) -> &Prim {
        self.joint.get_prim()
    }
}
...
mod tests {
    ...
    #[test]
    fn test_schema_kind() {
        assert_eq!(PrismaticJoint::SCHEMA_KIND, SchemaKind::ConcreteTyped);
    }

    #[test]
    fn test_schema_type_name() {
        assert_eq!(PrismaticJoint::SCHEMA_TYPE_NAME, "PhysicsPrismaticJoint");
    }

    #[test]
    fn test_schema_attribute_names() {
        let names = PrismaticJoint::get_schema_attribute_names(false);
