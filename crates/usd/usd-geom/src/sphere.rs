//! UsdGeomSphere - sphere geometry schema.
//!
//! Port of pxr/usd/usdGeom/sphere.h/cpp
//!
...
/// Sphere geometry schema.
///
/// Defines a primitive sphere centered at the origin.
///
/// Matches C++ `UsdGeomSphere`.
#[derive(Debug, Clone)]
pub struct Sphere {
    /// Base gprim schema.
    inner: Gprim,
...
impl Sphere {
    /// Creates a Sphere schema from a prim.
    ///
    /// Matches C++ `UsdGeomSphere(const UsdPrim& prim)`.
    ...
    /// Creates a Sphere schema from a Gprim schema.
    ///
    /// Matches C++ `UsdGeomSphere(const UsdSchemaBase& schemaObj)`.
    pub fn from_gprim(gprim: Gprim) -> Self {
        Self { inner: gprim }
    }

    /// Creates an invalid Sphere schema.
    pub fn invalid() -> Self {
        Self {
            inner: Gprim::invalid(),
        }
    ...
    /// Return a Sphere holding the prim adhering to this schema at path on stage.
    ///
    /// Matches C++ `UsdGeomSphere::Get(const UsdStagePtr &stage, const SdfPath &path)`.
    ...
    ///
    /// Matches C++ `UsdGeomSphere::Define(const UsdStagePtr &stage, const SdfPath &path)`.
    ...
    ///
    /// Matches C++ `ComputeExtent(double radius, VtVec3fArray* extent)`.
    pub fn compute_extent(radius: f64, extent: &mut [Vec3f; 2]) -> bool {
        extent[0] = Vec3f::new(-radius as f32, -radius as f32, -radius as f32);
        extent[1] = Vec3f::new(radius as f32, radius as f32, radius as f32);
        true
    }

    /// Compute the extent as if the matrix transform was first applied.
    ///
    /// Matches C++ `ComputeExtent(double radius, const GfMatrix4d& transform, VtVec3fArray* extent)`.
    pub fn compute_extent_with_transform(
        radius: f64,
        transform: &Matrix4d,
        extent: &mut [Vec3f; 2],
    ) -> bool {
        // Create bounding box from sphere range
        let range = Range3d::new(
            Vec3d::new(-radius, -radius, -radius),
            Vec3d::new(radius, radius, radius),
        );
    ...
    /// Compute the extent for the sphere at the specified time.
    ///
    /// Returns true on success, false if extent was unable to be calculated.
    /// On success, extent will contain the axis-aligned bounding box of the sphere.
    ///
    ...
    /// Compute the extent for the sphere at the specified time with transform.
    ///
    /// Returns true on success, false if extent was unable to be calculated.
    ///
    ...
    pub fn compute_extent_at_time_with_transform(
        ...
        let mut extent_array = [Vec3f::new(0.0, 0.0, 0.0), Vec3f::new(0.0, 0.0, 0.0)];
        if !Self::compute_extent_with_transform(radius, transform, &mut extent_array) {
            return false;
        }
    ...
    /// Compute the extent for the sphere at multiple times.
    ///
    /// Returns true on success, false if extent was unable to be calculated.
    ///
    ...
    /// Compute the extent for the sphere at multiple times with transform.
    ///
    /// Returns true on success, false if extent was unable to be calculated.
    ///
