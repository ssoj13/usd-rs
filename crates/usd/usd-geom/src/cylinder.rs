//! UsdGeomCylinder - cylinder geometry schema.
//!
//! Port of pxr/usd/usdGeom/cylinder.h/cpp
//!
...
/// Cylinder geometry schema.
///
/// Defines a primitive cylinder with closed ends, centered at the origin.
///
/// Matches C++ `UsdGeomCylinder`.
#[derive(Debug, Clone)]
pub struct Cylinder {
    /// Base gprim schema.
    inner: Gprim,
...
impl Cylinder {
    /// Creates a Cylinder schema from a prim.
    ///
    /// Matches C++ `UsdGeomCylinder(const UsdPrim& prim)`.
    ...
    /// Creates a Cylinder schema from a Gprim schema.
    ///
    /// Matches C++ `UsdGeomCylinder(const UsdSchemaBase& schemaObj)`.
    pub fn from_gprim(gprim: Gprim) -> Self {
        Self { inner: gprim }
    }

    /// Creates an invalid Cylinder schema.
    pub fn invalid() -> Self {
        Self {
            inner: Gprim::invalid(),
        }
    ...
    /// Return a Cylinder holding the prim adhering to this schema at path on stage.
    ///
    /// Matches C++ `UsdGeomCylinder::Get(const UsdStagePtr &stage, const SdfPath &path)`.
    ...
    ///
    /// Matches C++ `UsdGeomCylinder::Define(const UsdStagePtr &stage, const SdfPath &path)`.
    ...
    // ========================================================================
    // RadiusTop (Cylinder_1)
    // ========================================================================

    /// Returns the radiusTop attribute (Cylinder_1 schema).
    ///
    /// The radius of the top of the cylinder, i.e. the face in the
    /// direction of the positive axis.
    ///
    /// Matches C++ `UsdGeomCylinder_1::GetRadiusTopAttr()`.
    ...
    /// Creates the radiusTop attribute (Cylinder_1 schema).
    ///
    /// Matches C++ `UsdGeomCylinder_1::CreateRadiusTopAttr()`.
    ...
    // ========================================================================
    // RadiusBottom (Cylinder_1)
    // ========================================================================

    /// Returns the radiusBottom attribute (Cylinder_1 schema).
    ///
    /// The radius of the bottom of the cylinder, i.e. the face in the
    /// direction of the negative axis.
    ///
    /// Matches C++ `UsdGeomCylinder_1::GetRadiusBottomAttr()`.
    ...
    /// Creates the radiusBottom attribute (Cylinder_1 schema).
    ///
    /// Matches C++ `UsdGeomCylinder_1::CreateRadiusBottomAttr()`.
    ...
    /// Compute the extent for the cylinder defined by height, radius, and axis.
    ///
    ...
    pub fn compute_extent_with_transform(
        ...
        // Create bounding box from cylinder range
    ...
    /// Compute the extent for a Cylinder_1 (with different top/bottom radii).
    ///
    /// Matches C++ `UsdGeomCylinder_1::ComputeExtent(height, radiusTop, radiusBottom, axis, extent)`.
    pub fn compute_extent_cylinder1(
        height: f64,
        radius_top: f64,
        radius_bottom: f64,
        axis: &Token,
        ...
        Self::compute_extent(height, max_radius, axis, extent)
    }

    /// Compute the extent for a Cylinder_1 with transform applied.
    ///
    /// Matches C++ `UsdGeomCylinder_1::ComputeExtent(height, radiusTop, radiusBottom, axis, transform, extent)`.
    pub fn compute_extent_cylinder1_with_transform(
        height: f64,
        radius_top: f64,
        radius_bottom: f64,
        axis: &Token,
    ...
    /// Get the top radius at the specified time (Cylinder_1 schema).
    ///
    /// Matches C++ `UsdGeomCylinder_1::GetRadiusTop`.
    pub fn get_radius_top(&self, time: usd_sdf::TimeCode) -> Option<f64> {
        self.get_radius_top_attr().get_typed::<f64>(time)
    }

    /// Get the bottom radius at the specified time (Cylinder_1 schema).
    ///
    /// Matches C++ `UsdGeomCylinder_1::GetRadiusBottom`.
    ...
    /// Compute the extent for the cylinder at the specified time.
    ///
    /// Returns true on success, false if extent was unable to be calculated.
    /// On success, extent will contain the axis-aligned bounding box of the cylinder.
    ///
    ...
    /// Compute the extent for the cylinder at the specified time with transform.
    ///
    /// Returns true on success, false if extent was unable to be calculated.
    ///
    ...
    /// Compute the extent for the cylinder at multiple times.
    ///
    /// Returns true on success, false if extent was unable to be calculated.
    ///
    ...
    /// Compute the extent for the cylinder at multiple times with transform.
    ///
    /// Returns true on success, false if extent was unable to be calculated.
    ///
...
/// Schema for `UsdGeomCylinder_1` — cylinder with potentially different top/bottom radii.
///
/// C++ registers this as a separate prim type `"Cylinder_1"` distinct from `"Cylinder"`.
/// In the Rust port the attribute set is identical to `Cylinder`, but the schema type
/// name and `define()` type must use `"Cylinder_1"`.
///
/// Matches C++ `UsdGeomCylinder_1`.
#[derive(Debug, Clone)]
pub struct Cylinder1 {
    inner: Cylinder,
}

impl Cylinder1 {
    /// Creates a Cylinder1 schema from a prim.
    pub fn new(prim: Prim) -> Self {
        Self {
            inner: Cylinder::new(prim),
        }
    }

    /// Returns the schema type name, which is `"Cylinder_1"`.
    ///
    /// Matches C++ `TfType::AddAlias<UsdSchemaBase, UsdGeomCylinder_1>("Cylinder_1")`.
    pub fn schema_type_name() -> Token {
        Token::new("Cylinder_1")
    }

    /// Return a Cylinder1 holding the prim adhering to this schema at path on stage.
    pub fn get(stage: &Stage, path: &usd_sdf::Path) -> Self {
        if let Some(prim) = stage.get_prim_at_path(path) {
            Self::new(prim)
        } else {
            Self {
                inner: Cylinder::invalid(),
            }
        }
    }

    /// Ensure a prim of type `"Cylinder_1"` is defined at path.
    pub fn define(stage: &Stage, path: &usd_sdf::Path) -> Self {
        match stage.define_prim(path.get_string(), Self::schema_type_name().as_str()) {
            Ok(prim) => Self::new(prim),
            Err(_) => Self {
    ...
    /// Access underlying Cylinder schema.
...
mod tests {
    ...
    #[test]
    fn test_cylinder1_schema_type_name() {
        // Cylinder1 must report "Cylinder_1", not "Cylinder".
        assert_eq!(Cylinder1::schema_type_name().as_str(), "Cylinder_1");
        // Original Cylinder is still "Cylinder".
        assert_eq!(Cylinder::schema_type_name().as_str(), "Cylinder");
    }

    #[test]
    fn test_cylinder1_radius_top_bottom_attrs() {
        let names = Cylinder::get_schema_attribute_names(false);
    ...
    fn test_cylinder1_compute_extent() {
        ...
        // With equal radii, same as normal cylinder extent
        ...
        // max_radius = 1.5, half_height = 1.0
        assert_eq!(extent[1].x, 1.5_f32);
        assert_eq!(extent[1].z, 1.0_f32);
    }
}
